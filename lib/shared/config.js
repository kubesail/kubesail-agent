...
const config = {
  ...
  KUBESAIL_AGENT_INGRESS_CONTROLLER_PORT:
    parseInt(process.env.KUBESAIL_AGENT_INGRESS_CONTROLLER_PORT, 10) || 443,
  ...
  GATEWAY_ADDRESS: process.env.GATEWAY_ADDRESS || 'dev.k8g8.com',
  ...
  KUBESAIL_AGENT_GATEWAY_PORT:
    (process.env.KUBESAIL_AGENT_GATEWAY_TARGET || '').split(':')[1] || '8000',

  INGRESS_CONTROLLER_NAMESPACE:
    process.env.INGRESS_CONTROLLER_NAMESPACE ||
    ['traefik', 'kube-system', 'ingress-nginx', 'default', 'openshift-ingress'].join(','),
  ...
  KUBESAIL_AGENT_USE_INGRESS_CONTROLLER: !!process.env.KUBESAIL_AGENT_USE_INGRESS_CONTROLLER,

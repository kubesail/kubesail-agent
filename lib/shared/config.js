...
const config = {
  ...
  KUBESAIL_AGENT_GATEWAY_TARGET:
    process.env.KUBESAIL_AGENT_GATEWAY_TARGET || 'https://kubesail-gateway:8000', // Used to register with the gateway
  INGRESS_CONTROLLER_NAMESPACE:
    process.env.INGRESS_CONTROLLER_NAMESPACE || 'ingress-nginx,kube-system,default',
  INGRESS_CONTROLLER_ENDPOINT:
    process.env.INGRESS_CONTROLLER_ENDPOINT || 'ingress-nginx-controller,traefik,ingress-nginx',
  ...
  // ADDITION_CLUSTER_HOSTNAMES allows the Agent to redirect alternative hostnames to the kube api instead of to the ingress controller
  ADDITION_CLUSTER_HOSTNAMES: (process.env.ADDITION_CLUSTER_HOSTNAMES || '').split(','),

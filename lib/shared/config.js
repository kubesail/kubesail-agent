...
const config = {
  ...
  KUBESAIL_AGENT_GATEWAY_TARGET: process.env.KUBESAIL_AGENT_GATEWAY_TARGET || 'https://kubesail-gateway:8000', // Used to register with the gateway
  INGRESS_CONTROLLER_NAMESPACE: process.env.INGRESS_CONTROLLER_NAMESPACE || 'default',
  INGRESS_CONTROLLER_ENDPOINT: process.env.INGRESS_CONTROLLER_ENDPOINT || 'ingress-nginx',
  KUBESAIL_AGENT_HTTP_LISTEN_PORT: parseInt(process.env.KUBESAIL_AGENT_HTTP_LISTEN_PORT || 6000, 10), // Used for healthchecks internally on the agent cluster

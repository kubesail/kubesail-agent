...
const { LOG_LEVEL, LOGGING_LABEL, getRelease, AGENT_LOG_FORMAT } = require('../shared/config')
  ...
  format.json({
    replacer: (key, value) => {
      if (value instanceof Buffer) {
        ...
        return value.message || value.code || value.stack
      }
      return value
    }
  })
...
if (AGENT_LOG_FORMAT !== 'json') {
  const logFormat = printf(info => {
    ...
    const obj = JSON.stringify(c)
    return `[${info.timestamp}] ${info.level}: ${info.message} ${
      Object.keys(c).length > 0 ? obj : ''
    }`
  })

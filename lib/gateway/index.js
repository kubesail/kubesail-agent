...
class KsGateway {
  ...
  init = async () => {
    this.gatewayServer.use(bodyParser.urlencoded({ extended: false }))
    const bodyParserJson = bodyParser.json()
    this.gatewayServer.use(timeout(30000))
    this.gatewayServer.use(bodyParserJson)
    this.gatewayServer.use(
      morgan(LOG_FORMAT, {
    ...
    if (process.env.NODE_ENV !== 'development') {
      this.gatewayServer.use(
        helmet.hsts({ maxAge: 31536000000, includeSubDomains: true, force: true })
      )
    }

    this.gatewayServer.post('/agent/verify/:secret', async (req, res) => {
    ...
    this.gatewayServer.get('/agent/health/:secret/:agentKey', (req, res) => {
    ...
    this.gatewayServer.get('/agent/metrics/:secret', (req, res) => {
      if (req.params.agentKey && req.params.secret === KUBESAIL_API_SECRET) {
    ...
    this.gatewayServer.get('/health', (req, res) => {
      res.sendStatus(200)
    })
    ...
    this.gatewayHttpsReplyer.listen(INTERNAL_HTTPS_RESPONDER_PORT, '127.0.0.1', () => {
      this.gatewayServer.listen(GATEWAY_AGENT_LISTEN_PORT, () => {

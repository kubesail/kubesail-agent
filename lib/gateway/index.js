  ...
  agentRegistrationServer = https.createServer(
    ...
    (req, res) => {
      if (req.url === '/agent/verify') {
        ...
        req.on('end', () => {
          const { agentKey, agentSecret } = JSON.parse(buf)
          console.log({ agentKey, agentSecret })
          // Save agent config / reply to API here with config!

          this.agentRegistrationSocketServer.on('config-response', () => {
            console.log('got non-namespaced config response')
          })

          // io.sockets.clients('room')

          this.agentRegistrationSocketServer.on('config-response', socket => {
            console.log('got non-namespaced config response')
            console.log('socket.room', socket.rooms)
          })

          // TODO move response another route

...
class KsGateway {
  ...
  addSocketMapping = (socketID /*: Socket */, validDomains /*: Array<string> */) => {
    for (let i = 0; i < validDomains.length; i++) {
      const host = validDomains[i]
      if (host && isFQDN(host)) {
        this.agentRegistrationSocketServer.remoteJoin(socketID)
        // this.agentRegistrationSocketServer.sockets.adapter.remoteJoin()
  ...
  init = async () => {
    server.post('/agent/verify', async (req, res) => {
      ...
      const room = agentKey + '|' + agentSecret
      const sockets = await this.agentRegistrationSocketServer.in(room).clients((err, sockets) => {
        console.log({ sockets })
      })
      // this.addSocketMapping(socket, [clusterAddress])

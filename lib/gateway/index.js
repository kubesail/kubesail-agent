...
class KsGateway {
  ...
  gatewayHttpsReplyer = https.createServer(
    ...
    (req, res) => {
      logger.debug('gatewayHttpsReplyer got request!', { host: req.host, url: req.url })
      res.writeHead(503)
      res.end()
    }
  )
  ...
  async handleStream(
    ...
    if (socketStatus === NO_SOCKETS_CONNECTED) {
      logger.debug('handleStream() getSocketStatus returned NO_SOCKETS_CONNECTED', {
        host,
        protocol
      })
      ...
      writeHeader(socket, data, 503, protocol, SOCKET_CONNECTED_ELSEWHERE)
    } else if (socketStatus === SOCKET_CONNECTED_HERE) {
      logger.debug('handleStream() getSocketStatus returned SOCKET_CONNECTED_HERE', {
        host,
        protocol
      })

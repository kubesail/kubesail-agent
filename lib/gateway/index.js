...
class KsGateway {
  ...
  httpsProxyServer = net.createServer(socket => {
    ...
    socket.on('error', err => {
      // Piped TLS in Node.js generates spurious ECONNRESETs in some conditions.
      // See https://github.com/nodejs/node/issues/27916
  ...
  bindAgentSocketEvents = (
    ...
    socket.on('health-check', ({ agentVersion }) => {
      const req = https.request({ ...options, path: '/agent/config' }, res => {
        logger.info('agent health-check from api', { statusCode: res.statusCode })
      })
      req.write(
        JSON.stringify({ agentVersion, agentKey, agentSecret, gatewaySecret: KUBESAIL_API_SECRET })
      )
      req.end()
    })
    ...
    socket.on('config-response', ({ kubeConfig }) => {
      ...
      const req = https.request({ ...options, path: '/agent/config' }, res => {
        ...
        if (res.statusCode !== 200) {
          logger.error('Unable to save config to KubeSail API... Invalid AgentKey?')
        }
      })
      req.write(
  ...
  addSocketMapping = async (socketId /*: string */, validDomains /*: Array<string> */) => {
    await this.redis.set(socketId, GATEWAY_INTERNAL_ADDRESS)
    for (let i = 0; i < validDomains.length; i++) {
      const host = validDomains[i]
      if (host && isFQDN(host)) {
        logger.debug(`Adding mapping for ${host}`)
        // TODO: Allow multiple agents to serve a single hostname
        this.hostnameToSocketMapping[host] = socketId
        this.agentRegistrationSocketServer.of('/').adapter.remoteJoin(socketId, host, err => {
          if (err) {
            logger.error('Unable to remote join socket to hostname room', {
              host,
              errMsg: err.message
            })
          }
  ...
  getSocketStatus(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.hostnameToSocketMapping[host]) resolve({ status: SOCKET_CONNECTED_HERE })
        ...
        this.agentRegistrationSocketServer.in(host).clients(async (err, clients) => {
          if (err) reject(err)

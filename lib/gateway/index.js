...
class KsGateway {
  ...
  remoteJoin = util.promisify(this.agentRegistrationSocketServer.of('/').adapter.remoteJoin)
  ...
  addSocketMapping = async (
    ...
    for (let i = 0; i < validDomains.length; i++) {
      ...
      if (host && isFQDN(host)) {
        await this.remoteJoin(socketId, host)
        this.localSocketMapping[host] = socketId
        if (this.localSocketReverseMapping[socketId]) {
          this.localSocketReverseMapping[socketId].push(host)
        } else {
          this.localSocketReverseMapping[socketId] = [host]
        }
      } else {
        throw new Error(
  ...
  init = async () => {
    server.post('/agent/verify', async (req, res) => {
      const { agentKey, agentSecret, clusterAddress } = req.body
      ...
      const sockets = await util.promisify(
        this.agentRegistrationSocketServer.in(agentKey).clients
      )()

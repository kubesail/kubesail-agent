...
const { writeHeader, sampleArray, getWeek } = require('../shared')
...
class KsGateway {
  ...
  async handleStream(
    ...
    if (status === SOCKET_CONNECTED_HERE) {
      ...
      try {
        ...
        // Errors from socketIoStream are strings...
        logger.error('handleStream() socketIoStream failure!', { errMsg: err })
      ...
      socket.on('close', async () => {
        ...
        const currentBWSlot = date.getDay() * date.getHours() * date.getMinutes()
        ...
        await this.redis.zincrby(`${key}-rcv`, socket.bytesWritten, currentBWSlot)
        await this.redis.expireat(`${key}-rcv`, expireAt)

        await this.redis.zincrby(`${key}-sent`, socket.bytesRead, currentBWSlot)
      ...
      stream.on('error', err => {
        // Socketio stream seems to return an empty object literal on every connection?
        if (!err) return
        console.log('wat', typeof err, err)
        logger.warn('handleStream(SOCKET_CONNECTED_HERE) error on proxySocket:', {
          errMsg: err.message
        })
        socket.end()
      })
      ...
      try {
        ...
        logger.error('handleStream(SOCKET_CONNECTED_HERE) Socket-Stream pipe failure!', {
          errMsg: err.message,
          code: err.code,
          type: err.type
        })
  ...
  init = async () => {
    ...
    server.get('/agent/metrics/:secret', (req, res) => {
      if (req.params.agentKey && req.params.secret === KUBESAIL_API_SECRET) {
        // this.localSocketReverseMapping[socket.id]
    ...
    const redisAdapter = socketIoRedis({ pubClient: redisSession, subClient: redisSessionSub })

...
module.exports = function () {
  ...
  server.put('/agent/:secret/:agentKey', async (req, res) => {
    ...
    logger.silly('Got update to config', { agentKey, clusterAddress, firewall })
  ...
  server.put('/agent/watch/:startOrStop/:secret', async (req, res) => {
    ...
    try {
      ...
      logger.warn('existingWatches: Invalid data in redis agentwatch')
    }
    if (!existingWatches) existingWatches = []
    if (startOrStop === 'start') {
      existingWatches.push([username, namespace])
      await this.redis.setex(`agentwatch|${agentKey}`, 86400, JSON.stringify(existingWatches))
    } else if (startOrStop === 'stop') {
      const newWatches = existingWatches.filter(
        i => i.username !== username && i.namespace !== namespace
      )
      if (newWatches.length === 0) {
        await this.redis.del(`agentwatch|${agentKey}`)
      } else {
        await this.redis.setex(`agentwatch|${agentKey}`, 86400, JSON.stringify(newWatches))
      }
    }

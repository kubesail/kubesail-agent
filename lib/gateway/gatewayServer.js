...
const { LOG_FORMAT, KUBESAIL_API_SECRET, GATEWAY_ADDRESSES, RELEASE } = require('../shared/config')
...
module.exports = function () {
  ...
  server.post('/agent/verify/:secret', async (req, res) => {
    ...
    const { agentKey, clusterAddress, selectedGateway, email, wasCreated } = req.body
    ...
    if (selectedGateway && GATEWAY_ADDRESSES.includes(selectedGateway) === false) {
      logger.warn('I need to disconnect this agent and tell it to reconnect')
    }
  ...
  server.get('/agent/health/:secret/:agentKey', async (req, res) => {
    if (req.params.agentKey && req.params.secret === KUBESAIL_API_SECRET) {
      ...
      // If there is no health-data, we'll assume we're registering for the first time, and check again in a few ms
      if (!healthData) {
        await setPTimeout(250)
        ;[connected, healthData] = await this.redis.mget(`akhm|${agentKey}`, `healthcheck|${agentKey}`)
      }
      let healthCheckData = {}

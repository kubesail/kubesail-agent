...
const { LOG_FORMAT, KUBESAIL_API_SECRET, ALWAYS_VALID_DOMAINS } = require('../shared/config')
...
module.exports = function () {
  ...
  server.post('/agent/verify/:secret', async (req, res) => {
    ...
    this.messageAgent(agentKey, 'agent-data', {
      domains: [].concat(ALWAYS_VALID_DOMAINS).filter(Boolean),
      clusterAddress
    })
    // Request that the agent send its configuration
    this.messageAgent(agentKey, 'config-request')
    // Update the mapping for this agentKey
    await this.addAgentSocketMapping({ agentKey, domains: { clusterAddress: 0 } })
    logger.info('/agent/verify complete!', { agentKey, clusterAddress })
    res.sendStatus(202)
  })

  // A debug route - refresh the kube configuration
  server.put('/agent/config/request/:secret', async (req, res) => {
    if (req.params.secret !== KUBESAIL_API_SECRET) return res.sendStatus(403)
    const { agentKey, clusterAddress } = req.body
    this.messageAgent(agentKey, 'config-request', { clusterAddress })
  ...
  server.put('/agent/:secret/:agentKey', async (req, res) => {
    ...
    logger.info('Got update to config', { agentKey, domains })
    await this.addAgentSocketMapping({ agentKey, domains })
  ...
  server.delete('/agent/:secret/:agentKey', async (req, res) => {
    ...
    this.messageAgent(agentKey, 'remove-cluster')
    return res.sendStatus(200)
  })
  ...
  server.get('/agent/health/:secret/:agentKey', async (req, res) => {
    if (req.params.agentKey && req.params.secret === KUBESAIL_API_SECRET) {
      ...
      this.messageAgent(agentKey, 'health-check')
  ...
  server.put('/agent/watch/:startOrStop/:secret', async (req, res) => {
    ...
    this.messageAgent(agentKey, 'kube-watch', { username, startOrStop, namespace })

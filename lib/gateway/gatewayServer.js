...
module.exports = function () {
  ...
  server.post('/agent/verify/:secret', async (req, res) => {
    ...
    this.agentRegistrationSocketServer.in(agentKey).adapter.clients(async (err, sockets) => {
      ...
      for (const socketId of sockets) {
        await this.addSocketMapping(socketId, [clusterAddress])
      }
  ...
  server.put('/agent/:secret/:agentKey', async (req, res) => {
    if (req.params.agentKey && req.params.secret === KUBESAIL_API_SECRET) {
      ...
      logger.info('Got update to config', { agentKey, firewall, clusterAddress })
      if (firewall && firewall !== '0.0.0.0/0') {
        await this.redis.set(`fw:${clusterAddress}`, firewall)
      }
  ...
  server.delete('/agent/:secret/:agentKey', async (req, res) => {
    if (req.params.agentKey && req.params.secret === KUBESAIL_API_SECRET) {
      ...
      logger.debug(
        'Note that in development, you will be able to re-register the dev agent in 30 seconds'
      )
      await this.redis.setex(agentKey, process.env.NODE_ENV ? 30 : 3600 * 24, 'removed')
      this.agentRegistrationSocketServer.to(agentKey).emit('remove-cluster')
      return res.sendStatus(200)
    } else return res.sendStatus(403)
  })
  ...
  server.get('/agent/health/:secret/:agentKey', (req, res) => {
    if (req.params.agentKey && req.params.secret === KUBESAIL_API_SECRET) {
      ...
      this.agentRegistrationSocketServer.to(agentKey).emit('health-check')
      this.agentRegistrationSocketServer.in(agentKey).clients((err, clients) => {
        if (err) return res.sendStatus(500)
        return res.send({ connected: !!clients.length })
      })
    } else return res.sendStatus(403)
  })

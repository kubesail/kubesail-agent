...
module.exports = function () {
  ...
  server.put('/agent/:secret/:agentKey', async (req, res) => {
    ...
    logger.info('Got update to config', { agentKey, clusterAddress, firewall })
  ...
  server.get('/agent/health/:secret/:agentKey', async (req, res) => {
    if (req.params.agentKey && req.params.secret === KUBESAIL_API_SECRET) {
      ...
      const [connected, healthData] = await this.redis.mget(
        `akhm|${agentKey}`,
        `healthcheck|${agentKey}`
      )
      return res.send({ connected: !!connected, ...JSON.parse(healthData || {}) })

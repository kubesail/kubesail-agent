...
function addSocketMapping(socket /*: Net.Socket */, validDomains /*: Array<string> */) {
  for (let i = 0; i < validDomains.length; i++) {
    const domain = validDomains[i]
    if (domain && isFQDN(domain)) {
      logger.debug(`addSocketMapping() add registered socket for "${domain}"`)
      socket.join(domain)
    } else {
      throw new Error(
        `addSocketMapping cannot add non FQDN as a mapped socket! domain: "${domain}"`
      )
    }
  }
}
...
module.exports = function agentAuthHandler(
  ...
  req.on('error', e => {
    logger.error('Gateway failed to register agent with api!', {
      errMsg: e.message,
      code: e.code,
      type: e.type
    })

...
module.exports = class Gateway {
  ...
  async removeAgentSocket(agentKey /*: string */) {
    logger.debug('removeAgentSocket', { agentKey, hasSocketMapping: !!this.socketMap[agentKey] })
    if (this.socketMap[agentKey]) {
      for (const host in this.socketMap[agentKey].firewall) {
        await this.hostMap.del(host)
  ...
  async proxyToGateway(
    ...
    proxySocket.on(
      'error',
      proxyServerErrorHandler(proxySocket, 'proxySocket SOCKET_CONNECTED_ELSEWHERE')
    )
  }

...
module.exports = class Gateway {
  ...
  async addAgentSocketMapping(
    options /*: { agentKey: string|void, clusterAddress: string, socket: Socket|void, gateway: void|string, domains: Object } */
  ) {
    const { agentKey, gateway, domains } = options
  ...
  agentSocketConnectionHandler = async (
    ...
    if (status === 200) {
      ...
      let { clusterAddress, domains } = json
      if (!domains) domains = {}
      this.socketMap[agentKey] = socket
      logger.info('Agent registered! Sending configuration', { clusterAddress, domains })

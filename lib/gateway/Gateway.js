...
module.exports = class Gateway {
  ...
  async handleStream(
      ...
      try {
        ...
        if (proxyHeaders) {
          socket.once('data', async secondPacket => {
            ...
            const remainder = data.toString().split('\n')
            remainder.shift()
            this.handleStream(
              protocol,
              socket,
              Buffer.concat([Buffer.from(remainder.join('\n')), secondPacket]),
              proxyHeaders.remoteAddress
            )
          })
          return
    ...
    if (host && (host === GATEWAY_ADDRESS || host === 'kubesail-gateway')) {
      logger.debug('Forwarding request from ingress server to gateway-agent server', {
        host
      })

...
const gatewayVersion = fs
  .readFileSync('/app/' + require('../../package.json').version)
  .toString()
  .replace('\n', '')
...
module.exports = class Gateway {
      ...
      req.on('error', e => {
        ...
        if (retries >= maxApiRequestRetries) {
          if (e.code === 'ECONNREFUSED') {
            return setTimeout(() => {
              resolve(this.kubesailApiReqest(method, path, data, ++retries))
            }, retries * 1000)
          }
        }
  ...
  async addAgentSocketMapping(
    options /*: { agentKey: string|void, clusterAddress: string, socket: Socket|void, gateway: void|string, firewall: Object } */
  ) {
    const { agentKey, gateway, firewall = {} } = options
    ...
    logger.debug('addAgentSocketMapping', {
      agentKey,
      clusterAddress,
      gateway,
      firewall,
    ...
    if (socket) socket.emit('agent-data', saveData)
  }
  ...
  agentSocketConnectionHandler = async (
    ...
    socket.on('disconnect', async reason => {
      if (this.socketMap[agentKey]) {
        this.kubesailApiReqest('PUT', '/agent/disconnect', {
          agentKey,
          agentSecret,
    ...
    socket.on('config-response', async ({ kubeConfig }) => {
      ...
      try {
        ...
        logger.error('Failed to post /agent/config (config-response)')
        socket.disconnect()
      }
    })

    socket.on('health-check', async ({ agentVersion, automated = false }) => {
      const socketMapData = this.socketMap[agentKey]
      if (!socketMapData) {
        logger.warn(
          'Recieved health-check for socket that we did not have a socket-mapping for! Disconnecting!'
        )
        return socket.disconnect()
      }
      if (!this.hostMap.get(socketMapData.clusterAddress)) {
        logger.warn(
          'Recieved health-check for socket that had a socketMap, but not a hostMap! Disconnecting!',
          {
            clusterAddress: socketMapData.clusterAddress,
            socketMapData
          }
        )
        return socket.disconnect()
      ...
      logger.debug('Received health-check!', { agentKey, agentVersion, automated })
      await this.redis.setex('healthcheck|' + agentKey, 120, Date.now().toString())
      if (!automated) {
        try {
          await this.kubesailApiReqest('POST', '/agent/config', {
            agentKey,
    ...
    try {
      const resp = await this.kubesailApiReqest('POST', '/agent/register', {
        username,
        agentKey,
        agentSecret,
        gatewaySecret: KUBESAIL_API_SECRET,
      ...
      logger.error('Failing to post /agent/register to kube api!', { errMsg: err.message })
    }

    if (status === 200) {
      // AGENT_REGISTER_VALID
      let { clusterAddress, firewall } = json
      if (!firewall) firewall = {}
      socket.clusterAddress = clusterAddress
      this.socketMap[agentKey] = socket
      logger.info('Agent registered! Sending configuration', { clusterAddress, firewall })
      ...
      let { clusterAddress } = json
      logger.info('New agent pending', { agentKey })
      socket.clusterAddress = clusterAddress
      this.socketMap[agentKey] = socket
    } else if (status >= 400 && status < 500) {
      logger.debug('Disconnected agent due to rejected agentSocketConnectionHandler reply', {
        status
      })
  ...
  async proxyToGateway(
    ...
    proxySocket.on('error', async err => {
      if (err.code === 'ETIMEDOUT') {
        logger.info('proxySocket ETIMEDOUT', { hostData })
        socket.end()
        await this.messageAgent(agentKey, 'health-check')
        setTimeout(async () => {
          const checkin = await this.redis.get('healthcheck|' + agentKey)
          if (!checkin) {
            logger.info('Agent is not responding to health-checks, cleaning up agent-data!', {
              hostData
            })
            await this.removeAgentSocket(agentKey)
          }
  ...
  async handleStream(
    ...
    if (!host || !isFQDN(host)) {
      logger.silly('Received request with invalid host header', { protocol, host })

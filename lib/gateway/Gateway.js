...
function proxyServerErrorHandler(sockets = [], name) {
  return err => {
    let level = 'warn'
    ...
    logger[level](`proxyServerErrorHandler: ${name}: error:`, {
      ...
      code: err.code,
      type: err.type,
      clusterAddress: (sockets.find(s => s.__clusterAddress) || {}).__clusterAddress
    })
    for (let i = 0; i < sockets.length; i++) {
      try {
        sockets[i].end()
      } catch (err) {
        logger.error('proxyServerErrorHandler failed to close socket', {
          message: err.message,
          stack: err.stack
        })
      }
...
module.exports = class Gateway {
  ...
  async proxyToGateway(
    ...
    proxySocket.connect(
      ...
      () => {
        ...
        proxySocket
          ...
          .on(
            'error',
            proxyServerErrorHandler([socket, proxySocket], 'socketPipeErr SOCKET_CONNECTED_ELSEWHERE')
          )
          .pipe(proxySocket)
          .on('error', proxyServerErrorHandler([socket, proxySocket], 'streamErr SOCKET_CONNECTED_ELSEWHERE'))
      }
    )
    socket.on('close', () => {
      try {
    ...
    socket.on('error', proxyServerErrorHandler([socket, proxySocket], 'socket SOCKET_CONNECTED_ELSEWHERE'))
    proxySocket.on('error', async err => {
      if (err.code === 'ETIMEDOUT') {
        ...
        proxyServerErrorHandler([socket, proxySocket], 'proxySocket SOCKET_CONNECTED_ELSEWHERE')(err)
      }
    })
  }
  ...
  async proxyLocalSocket(
    ...
    stream.on('error', proxyServerErrorHandler([socket, stream], 'streamErr'))
    ...
    socket
      .pipe(stream)
      .on('error', proxyServerErrorHandler([stream, socket], 'streamPipeErr SOCKET_CONNECTED_HERE'))
      .pipe(socket)
      .on('error', proxyServerErrorHandler([stream, socket], 'socketPipeErr SOCKET_CONNECTED_HERE'))
  }
  ...
  async handleStream(protocol /*: string */, socket /*: Socket */, data /*: any */) {
    ...
    if (!result) {
      logger.debug('handleStream: no result from getAgentSocketStatus', { host, hostMap: this.hostMap.keys() })

...
module.exports = class Gateway {
  ...
  async addAgentSocketMapping({
    ...
    if (socket && !gateway) {
      logger.debug('addAgentSocketMapping', { agentKey, GATEWAY_INTERNAL_ADDRESS })
    ...
    for (const host in domains) {
      logger.debug('Adding mapping', { host, saveData })
  ...
  async getAgentSocketStatus(host /*: string */) /*: false|Object */ {
    ...
    if (!hostData) {
      ...
      if (fromDb) {
        ...
        logger.debug('Unable to find host data', { host })
        return false
      }
    }

...
module.exports = class Gateway {
  ...
  async handleStream(
	...
	const proxyProtocol = stringData.indexOf('PROXY TCP4')
	const isQUITline = stringData.substr(0, 11) === '\r\n\r\n\u0000\r\nQUIT'
        if (!isQUITline && (proxyProtocol > -1 || proxyHeaders.remoteAddress)) {
          const proxyRaw = stringData.substr(proxyProtocol, data.length)
          const proxyParts = proxyRaw.split(' ')
	  const remoteAddr = proxyHeaders.remoteAddress || proxyParts[3]
          if (remoteAddr) {
            socket.once('data', secondPacket => {
              logger.silly(
                'Forwarding second packet back to handleStream with remote IP from PROXY header',
                { remoteAddr, data: data.toString('utf8'), secondPacket: secondPacket.toString('utf8') }
              )
              this.handleStream(
                protocol,
                socket,
    ...
    if (!host || !isFQDN(host)) {
      logger.silly('handleStream: Received request with invalid host header or SNI header', {
        protocol,
        host,
	data: data.toString('utf8'),
	len: data.length
    ...
    if (result.socket) {
      ...
      return writeHeader(socket, data, 502, protocol, 'KS_GATEWAY_NO_AGENT_CONNECTED_CLEANUP')
    }
  }

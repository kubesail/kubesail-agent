...
const sniReader = require('sni-reader')
...
const proxyProtocolParser2 = require('proxy-protocol-js')
...
module.exports = class Gateway {
  ...
  httpProxyServer = net.createServer(socket => {
  ...
  httpsProxyServer = net.createServer(socket => {
    sniReader(socket, (err, serverName) => {
      console.log('from SNIREADER:', { serverName })
  ...
  async handleStream(
    ...
    try {
      ...
      const proxyHeaders2 = proxyProtocolParser2.V2ProxyProtocol.parse(data)

      console.log({ proxyHeaders, proxyHeaders2 })

      const isQUITline = stringData.substr(7, 11) === 'QUIT'
	  ...
	  remoteAddr = proxyHeaders.remoteAddress || proxyParts[3]
        } else if (!isQUITline) {
          logger.silly(

...
module.exports = class Gateway {
  ...
  async addAgentSocketMapping(
    ...
    if (socket) socket.emit('agent-data', { ...saveData, gateway: undefined })
  }
  ...
  agentSocketConnectionHandler = async (
    ...
    socket.on('health-check', async ({ agentVersion, automated = false }) => {
      ...
      logger.silly('Received health-check!', { agentKey, hasHostMapping, agentVersion, automated })

      await this.redis.setex('healthcheck|' + agentKey, 120, Date.now().toString())
      if (!automated && hasHostMapping) {
        try {
          await this.kubesailApiReqest('POST', '/agent/config', {
            agentKey,
            agentSecret,

...
module.exports = class Gateway {
  ...
  httpProxyServer = net.createServer({ noDelay: true, keepAlive: true }, async socket => {
  ...
  httpsProxyServer = net.createServer({ noDelay: true, keepAlive: true }, socket => {
  ...
  agentSocketConnectionHandler = async (
    ...
    socket.on('disconnect', async reason => {
      if (this.socketMap[agentKey] && !this.socketMap[agentKey].connected) {
        ...
        logger.silly('Agent disconnect event fired but agent is still connected (quick reconnect)', {
          reason,
          agentKey
        })
      }

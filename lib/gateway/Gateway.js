...
module.exports = class Gateway {
  ...
  async getAgentSocketStatus(host /*: string */) /*: false|Object */ {
    let hostData = await this.readAgentSocketStatus(host)
    if (!hostData) {
      // Wildcard support
      const hostNameParts = host.split('.') // ['foo', 'bar', 'com']
      while (hostNameParts.shift() && !hostData) {
        if (hostNameParts.length < 2) break
        hostData = await this.readAgentSocketStatus(['*', ...hostNameParts].join('.')) // *.bar.com
      }
    }
    if (!hostData) {
  ...
  async handleStream(
    ...
    if (!result) {
      ...
      logger.warn('handleStream: unknown status from getSocketStatus, disconnecting', {
        host,
        result,
        GATEWAY_INTERNAL_ADDRESS,

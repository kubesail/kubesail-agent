...
module.exports = class Gateway {
  ...
  agentSocketConnectionHandler = async (
    ...
    if (!agentKey) {
      logger.warn('agentSocketConnectionHandler: Rejecting agent with no agentKey')
      socket.disconnect()
    }

    if (this.socketMap[agentKey]) {
      logger.warn('agentSocketConnectionHandler: Agent is already in socket map? Rejecting connection...', {
        agentKey
      })
      socket.disconnect()
    }
    ...
    socket.on('disconnect', async reason => {
      if (this.socketMap[agentKey] && !this.socketMap[agentKey].connected) {
        ...
        logger.debug('Agent disconnect event fired but agent is still connected (quick reconnect)', {
          reason,
          agentKey
        })
      }
    ...
    if (!agentKey && !agentSecret) {
      ...
      try {
        await kubesailApiRequest({
          ...
          data: { pendingKey, gatewaySecret: KUBESAIL_API_SECRET, gatewayAddress: GATEWAY_ADDRESS }
        })
      } catch (err) {

...
const proxyProtocolParser = require('@balena/proxy-protocol-parser')
...
module.exports = class Gateway {
  ...
  httpProxyServer = net.createServer(async socket => {
    socket.once('data', async data => this.handleStream('http', socket, data))
  ...
  httpsProxyServer = net.createServer(socket => {
    socket.once('data', async data => this.handleStream('https', socket, data))
  ...
  async handleStream(
    ...
    if (!remoteAddr) {
      try {
        const proxyHeaders = proxyProtocolParser.decode(data)
        if (proxyHeaders) {
          socket.once('data', async secondPacket => {
            logger.silly(
              'Forwarding second packet back to handleStream with remote IP from PROXY header',
              { remoteAddr: proxyHeaders.remoteAddress }
            )
            this.handleStream(protocol, socket, secondPacket, proxyHeaders.remoteAddress)
          })
          return
        } else if (protocol === 'http' && process.env.NODE_ENV !== 'development') {
          logger.debug(
            'handleStream: received TLS request with no remoteAddr and no proxyHeaders were parsable. PROXY Protocol disabled?',
            {
        ...
        logger.error('handleStream: Failed to parse PROXY protocol headers! Continuing.', {
          errMsg: err.message,
          stack: err.stack,
          code: err.code,
          data: data.toString('ascii'),
    ...
    if (!remoteAddr) {
      remoteAddr = socket.remoteAddress.substring(
        socket.remoteAddress.lastIndexOf(':') + 1,
        socket.remoteAddress.length
      )
      const range = ipAddr.parse(remoteAddr).range()
      if (range === 'private' || range === 'loopback') {
        if (process.env.NODE_ENV === 'development') {
          logger.silly(
            'handleStream: No PROXY protocol detected - in production this request would be rejected. Accepting due to development mode.',
            { range, remoteAddr, host, protocol }
          )
        } else {
          logger.error(
            'handleStream: Received request from local address! Possibly PROXY protocol is not configured? Rejecting request!',
            { range, remoteAddr, host, protocol, data: data.toString('ascii') }
          )
          return writeHeader(socket, data, 501, protocol, 'KS_GATEWAY_NO_HOST')
        }
      }
    ...
    if (!result) {
      ...
      return writeHeader(socket, data, 502, protocol, 'KS_GATEWAY_NO_AGENT_CONNECTED')
    }
    ...
    try {
      ...
      } else logger.silly('Firewall ACCEPT!', debugInfo)
    } catch (err) {
      throw new Error(
        'Failed to construct or test CIDRMatcher. Debug info: ' +
          JSON.stringify(
            { errMsg: err.message, code: err.code, stack: err.stack, ...debugInfo },
            null,
            4
          )
      )
    ...
    if (result.socket) {
      this.proxyLocalSocket(result, protocol, hostname, socket, remoteAddr, data)

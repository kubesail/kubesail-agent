...
const { writeHeader, getWeek, kubesailApiReqest } = require('../shared')
const {
  TLS_KEY_PATH,
  TLS_CERT_PATH,
  TLS_CHAIN_PATH,
...
module.exports = class Gateway {
  ...
  agentSocketConnectionHandler = async (
    ...
    socket.on('disconnect', async reason => {
      if (this.socketMap[agentKey]) {
        kubesailApiReqest('PUT', '/agent/disconnect', {
          agentKey,
          agentSecret,
    ...
    socket.on('config-response', async ({ kubeConfig }) => {
      ...
      try {
        await kubesailApiReqest('POST', '/agent/config', {
          agentKey,
          agentSecret,
          gatewaySecret: KUBESAIL_API_SECRET,
          kubeConfig
    ...
    socket.on('health-check', async (data = {}) => {
      ...
      logger.debug('Received health-check!', { agentKey, hasHostMapping, ...data })
    ...
    try {
      const resp = await kubesailApiReqest('POST', '/agent/register', {
        username,
        agentKey,
        agentSecret,
        gatewaySecret: KUBESAIL_API_SECRET,

...
const { writeHeader, kubesailApiReqest } = require('../shared')
const {
  TLS_KEY_PATH,
  TLS_CERT_PATH,
  TLS_CHAIN_PATH,
...
module.exports = class Gateway {
  ...
  async addAgentSocketMapping(
    ...
    for (const host in firewall) {
      logger.debug('addAgentSocketMapping', { agentKey, host })
  ...
  async getAgentSocketStatus(host /*: string */) /*: false|Object */ {
    ...
    if (hostData) return hostData
      ...
      logger.error('Unable to determine socket status!', { host })
      return false
    }
  }
  ...
  messageAgent = async (agentKey /*: string */, key /*: string */, value /*: any */) => {
    if (this.socketMap[agentKey]) {
      ...
      logger.debug('messageAgent publishing to redis:', { agentKey, key, value })
  ...
  async proxyLocalSocket(
    ...
    socket.on('close', async () => {
      stream.end()
      bandwidthRecv.set({ agentKey: hostData.agentKey }, socket.bytesWritten)
      bandwidthSent.set({ agentKey: hostData.agentKey }, socket.bytesRead)
    })
  ...
  async handleStream(
    ...
    if (!result) {
      ...
      writeHeader(socket, data, 502, protocol, 'NO_SOCKETS_CONNECTED_CLEANUP')
    }
  }

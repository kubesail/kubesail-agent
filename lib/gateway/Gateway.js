...
module.exports = class Gateway {
  ...
  agentSocketConnectionHandler = async (
    ...
    if (status === 200) {
      ...
      let existingWatches = []
      try {
        existingWatches = JSON.parse(await this.redis.get(`agentwatch|${agentKey}`))
      } catch (err) {
        logger.warn('agentRegister: Invalid data in redis agentwatch')
      }
      for (const watch of existingWatches) {
        await this.messageAgent(agentKey, 'kube-watch', {
          username: watch[0],
          namespace: watch[1],
          startOrStop: start
        })

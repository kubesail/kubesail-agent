...
module.exports = class Gateway {
  ...
  httpProxyServer = net.createServer({ noDelay: false, keepAlive: true }, async socket => {
    socket.on('error', err => {
      logger.error('httpProxyServer: Unexpected error on socket', {
        errMsg: err.message,
        code: err.code
      })
      socket.end()
  ...
  httpsProxyServer = net.createServer({ noDelay: false, keepAlive: true }, socket => {
    socket.on('error', err => {
      logger.error('httpsProxyServer: Unexpected error on socket', {
        errMsg: err.message,
        code: err.code
      })
      socket.end()
  ...
  async proxyLocalSocket(
    ...
    stream.on('error', err => {
      logger.error('proxyLocalSocket: socketIoStream error!', {
        agentKey: hostData.agentKey,
        errMsg: err.message,
        code: err.code,
        remoteAddr

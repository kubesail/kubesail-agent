...
module.exports = class Gateway {
  ...
  async handleStream(
    ...
    const firstLine = data.toString('ascii')
    if (firstLine.startsWith('PROXY TCP4 ')) {
      const remoteIpAddress = firstLine.split(' ')[2]
      socket.once('data', async secondPacket => {
        logger.debug(
          'Forwarding second packet back to handleStream with remote IP from PROXY header',
          { remoteIpAddress }
        )
        this.handleStream(protocol, socket, secondPacket, remoteIpAddress)
      })
      return
    }

...
module.exports = async function handleStream(
  ...
  if (status === SOCKET_CONNECTED_HERE) {
    ...
    if (!firewall || !matcher.contains(ipv4)) {
      logger.debug('Firewall REJECT!', { host, firewall, ipv4 })
      return writeHeader(socket, data, 501, protocol, 'UNSUPPORTED_PROTOCOL')
    }
    ...
    proxySocket.connect(
      ...
      () => {
        ...
        proxySocket
          .pipe(socket)
          .on('error', onError('socketPipeErr SOCKET_CONNECTED_ELSEWHERE'))
          .pipe(proxySocket)
          .on('error', onError('streamErr SOCKET_CONNECTED_ELSEWHERE'))
      }
    )
    socket.on('close', () => {
      try {

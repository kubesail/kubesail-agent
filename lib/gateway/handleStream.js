...
module.exports = async function handleStream(
  ...
  if (status === SOCKET_CONNECTED_HERE) {
    logger.info('handleStream() getSocketStatus returned SOCKET_CONNECTED_HERE', debugInfo)
    ...
    if (!matcher.contains(ipv4)) {
      logger.info('Firewall REJECT!', { firewall, ipv4 })
      return writeHeader(socket, data, 501, protocol, 'UNSUPPORTED_PROTOCOL')
    }
    ...
    socket.on('error', err => {
      if (err.code !== 'EPIPE') {
        logger.warn('handleStream(SOCKET_CONNECTED_HERE) error on socket:', {
          errMsg: err.message,
          code: err.code,
          type: err.type
        })
    ...
    stream.on('error', err => {
      if (err.code !== 'EPIPE') {
        logger.warn('handleStream(SOCKET_CONNECTED_HERE) error on stream:', {
          errMsg: err.message,
          code: err.code,
          type: err.type
        })
    ...
    logger.info('handleStream() getSocketStatus returned NO_SOCKETS_CONNECTED', debugInfo)

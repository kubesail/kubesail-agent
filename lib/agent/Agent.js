...
class KubesailAgent {
  ...
  httpsServer = https.createServer(
    ...
    async (req, res) => {
      const { ip, port } = await this.k8s.findServiceForRequest(req, res)
      this.reverseProxy.web(req, res, { target: `http://${ip}:${port}` })
    }
  )
  ...
  async updateHostMap() {
    ...
    logger.debug('updateHostMap:', { desiredHostMap, current: this.currentHostMap })
  ...
  k8s = {
    ...
    findServiceForRequest: async (req, res) => {
      ...
      const ingress = this.docs.find(
        i => i.kind === 'Ingress' && i.spec.rules.find(r => r.host === host)
      )

...
const tlsStoreLRU = new LRU({ max: 500 })
...
class KubesailAgent {
  ...
  reverseProxyRequest = (req, options) => {
    options.headers['x-forwarded-for'] = req.socket.remoteAddress
    options.headers['x-forwarded-proto'] = 'https'
    options.headers['x-forwarded-host'] = req.headers.host || req.headers[':authority'].split(':')[0]
    // delete options.headers['accept-encoding']
    // delete options.headers['dnt']

    return http.request({ ...options, agent: this.keepAliveAgentHTTP })
  }
  reverseProxyResponse = (req, res, proxyRes) => {
  ...
  async init() {
    ...
    this.httpsServer.on('newSession', (id, data, cb) => {
      tlsSessionStore[id.toString('hex')] = data
      cb()
    })
    this.httpsServer.on('resumeSession', (id, cb) => {
      if (tlsSessionStore[id.toString('hex')]) {
        console.log('Session resumed?!')
      }
      cb(null, tlsSessionStore[id.toString('hex')] || null)
    })
    this.httpsServer.on('secureConnection', tlsSocket => {
      console.log('secure connection reuse:', {
      ...
      if (result?.ip && result?.port) {
        ...
        http2Proxy.web(
          ...
          res,
          {
            ...
            protocol: 'http:',
            proxyTimeout: 10000,
            onReq: this.reverseProxyRequest,
            onRes: this.reverseProxyResponse
          },
          err => {
            console.log('proxied', {
              totalSocketCount: this.keepAliveAgent.totalSocketCount,
              events: this.keepAliveAgent._eventsCount,
              totalSocketCount2: this.keepAliveAgentHTTP.totalSocketCount,
              events2: this.keepAliveAgentHTTP._eventsCount
            })
            if (err) {
              const debugObj = {
                errMsg: err.message,
                ...
                port: result.port,
                hostname: result.ip,
                ingress: req._ingress?.metadata?.name,
                stack: err.stack
              }
    ...
    this.httpsServer.on('sessionError', err => {
      console.log('server sessionError', err.toString())
    })
    this.httpsServer.on('tlsClientError', err => {
      console.log('server tlsClientError', err.toString())
    })
  ...
  reconnect = async (time = 2000) => {
    ...
    time = Math.min(time + Math.floor(Math.random() * Math.floor(time)) * this.reconnectionCount, 8000)
  ...
  async installCertificate(namespace, secretName, hostname) {
    ...
    if (secret) {
      ...
      const certs = Buffer.from(secret.body.data['tls.crt'], 'base64')
        .toString('ascii')
  ...
  k8s = {
    ...
    filterKubeEvents: event => {
      if (event && event.object && event.object.metadata) {
        if (event.object.kind === 'Endpoint' || event.object.kind === 'Endpoints') {
          ...
          logger.silly('watchAll: sending event:', {
            type: event.type,
      ...
      if (['Ingress', 'Service', 'Endpoints', 'Secret', 'Deployment'].includes(doc.kind)) {
        ...
        if (eventType === 'ADDED') {
          ...
          const newPile = this.docs.map(d => {
            if (
              docMatch(d) &&
              parseInt(doc.metadata.resourceVersion, 10) > parseInt(d.metadata.resourceVersion, 10)
            ) {
              madeChange = true
              return doc
            } else return d
          })
          if (madeChange) this.docs = newPile
    ...
    findServiceForRequest: async (req, res) => {
      ...
      return { host, port: port.port, ip: service.spec.clusterIP, service, ingress }
    },
    watchResources: () => {
      const connect = async (group, version, kind) => {
        ...
        stream.on('data', async event => {
          ...
          logger.silly('Kubernetes Event', {
            userWantsEvent: !!userWantsEvent,
            filtered,
            wouldSend: userWantsEvent && filtered,
            e:
              event.type +
              ' ' +
              event.object.kind +

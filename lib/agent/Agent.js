...
class KubesailAgent {
  ...
  reverseProxyRequest = (req, options) => {
    ...
    return http.request({ ...options, agent: this.keepAliveAgent })
  }
          ...
          err => {
            if (err) {
              ...
              if (['ECONNRESET', 'ECONNREFUSED'].includes(err.code)) {
                ...
                return res.end(`KS_PROXY_APP_ERROR`)
              }
            }
          }
        )
  ...
  async updateHostMap() {
    ...
    if (_.isEqual(desiredHostMap, this.currentHostMap)) {
      logger.debug('updateHostMap: No action required, host map already up-to-date', { desiredHostMap })
      return
    }
  ...
  k8s = {
    ...
    trackResource: (eventType, doc) => {
      ...
      if (['Ingress', 'Service', 'Endpoints', 'Secret', 'Deployment'].includes(doc.kind)) {
        ...
        logger.silly(`k8s: ${eventType} ${doc.kind}/${doc.metadata.name}`)
        if (eventType === 'ADDED') {
          ...
          const existing = this.docs.findIndex(
            d =>
              docMatch(d) &&
              parseInt(doc.metadata.resourceVersion, 10) > parseInt(d.metadata.resourceVersion, 10)
          )
          madeChange = existing > -1
          if (existing > -1) this.docs.splice(existing, 1)
    ...
    findServiceForRequest: async (req, res) => {
      ...
      //   if (res) {
      //     logger.debug('requestHandler: App not yet ready?', { endpoint, serviceSelector: service.spec.selector })

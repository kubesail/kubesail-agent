    ...
    lookup: (hostname, _opts, cb) => {
      if (isFQDN(hostname)) {
        ...
        dns.lookup(hostname, { all: true }, (err, results) => {
          if (err) {
            logger.error('lookup: Failed to lookup hostname', {
              hostname,
              errMsg: err.message,
              code: err.code
            })
  ...
  kubesailApiRequest = (
    ...
    return new Promise((resolve, reject) => {
      ...
      const errHandler = e => {
        ...
        reject(
          new Error(
            `Failed to post to KubeSail API: ${method} ${KubeSailApiTarget}:${KubeSailApiPort}${path}. Received "${e.code}" after ${retries} tries`
          )
        )
      }

...
const { setPTimeout } = require('../shared')
...
// Increase the length of the stacktraces so we can debug better (default is 10)
...
class KubesailAgent {
  ...
  dns = {
    ...
    setServers: async () => {
      ...
      try {
        if (!isFQDN(KUBESAIL_AGENT_GATEWAY_TARGET) || KUBESAIL_AGENT_GATEWAY_TARGET.endsWith('.default')) {
          logger.debug('Skipping setResolver: KUBESAIL_AGENT_GATEWAY_TARGET is not an FQDN', {
            KUBESAIL_AGENT_GATEWAY_TARGET
          })
          return
        }
        const resolved = await dns.resolve4(KUBESAIL_AGENT_GATEWAY_TARGET)
        if (resolved) {
          logger.debug('Successfully resolved DNS address for agent target:', {
            target: KUBESAIL_AGENT_GATEWAY_TARGET,
            resolved
          })
        } else revertToFallback()
      } catch (err) {
        revertToFallback(err)
      }
    }
  }
  ...
  async init() {
    ...
    let checkPublicIpInterval
    let checkK8g8CertInterval
    this.registerWithGateway().then(async () => {
      ...
      if (checkPublicIpInterval) clearInterval(checkPublicIpInterval)
      checkPublicIpInterval = setInterval(async () => {
        await this.checkPublicIP()
      }, 15 * 60 * 1000)

      if (checkK8g8CertInterval) clearInterval(checkK8g8CertInterval)
      checkK8g8CertInterval = setInterval(async () => {
  ...
  async checkPublicIP() {
    ...
    try {
      const { body } = await KubesailAgent.kubesailApiRequest(
        `/whatsmyip/${privateIP}`,
        'PUT',
        null,
        0,
        ...
        if (e.code === 'ECONNRESET' || e.code === 'ECONNREFUSED' || e.code === 'EAI_AGAIN') {
          ...
          if (retries < retryLimit) {
            return safeTimers.setTimeout(() => {
              resolve(KubesailAgent.kubesailApiRequest(path, method, data, ++retries))
            }, (retries + 1) * 1000)
          }
        }
        reject(
          new Error(
            `Failed to post to KubeSail API: ${method} ${KubeSailApiTarget}:${
              KubeSailApiPort || 443
            }${path}. Received "${e.code}" after ${retries} tries. Stack:\n${e.stack}`
          )
        )
      }
      logger.silly('kubesailApiRequest:', { path, method })
      const req = https.request({ ...options, path, agent: keepAliveAgentHTTPS }, res => {
    ...
    return new Promise((resolve, _reject) => {
      ...
      this.gatewaySocket.on('kube-watch', async ({ username, namespace, startOrStop }) => {
        if (startOrStop === 'start') {
          ...
          //   try {
          //     await KubesailAgent.kubesailApiRequest('/agent/metrics', 'POST', {
          //       agentKey: KUBESAIL_AGENT_KEY,
      ...
      this.gatewaySocket.on('disk-stats', async () => {
        ...
        if (!res || res.statusCode !== 200) {
          ...
          await KubesailAgent.kubesailApiRequest('/agent/disk-stats/error', 'POST', {
            agentKey: KUBESAIL_AGENT_KEY,
            agentSecret: KUBESAIL_AGENT_SECRET
          })
          return
        ...
        res.on('end', async () => {
          try {
            ...
            if (diskStats) {
              await KubesailAgent.kubesailApiRequest('/agent/disk-stats', 'POST', {
                agentKey: KUBESAIL_AGENT_KEY,
                agentSecret: KUBESAIL_AGENT_SECRET,
                diskStats
              })
  ...
  async installCertificate(namespace, secretName, hostname, secret) {
    ...
    if (!secret) {
      try {
        ...
        if (err.code !== 404) {
          logger.warn('installCertificate: Unable to get latest version of secret', {
            namespace,
            secretName,
            code: err.code
          })
  ...
  async checkK8g8CertificateExpiration(certificate) {
    ...
    // Renew certs when they have 2 days left
    const renewsWhenCertExpiresIn = 1000 * 60 * 60 * (24 * 2)
    const needsRenewal = expiresIn < renewsWhenCertExpiresIn
    const expiresHuman = `${Math.round(expiresIn / 1000 / 60 / 60, 1)}h`
    const renewHuman = `${Math.round(renewsWhenCertExpiresIn / 1000 / 60 / 60, 1)}h`
    ...
    if (needsRenewal) {
      ...
      logger.debug('checkK8g8CertificateExpiration: Certificate still valid for', {
        expiresIn: expiresHuman,
        renewsWhenCertExpiresIn: renewHuman
      })
    }
    return needsRenewal
  }
  ...
  async updateHostMap() {
    ...
    const { res, body } = await KubesailAgent.kubesailApiRequest('/agent/host-mapping-request', 'POST', {
      agentKey: KUBESAIL_AGENT_KEY,
      agentSecret: KUBESAIL_AGENT_SECRET,
    ...
    findServiceForRequest: async (req, res) => {
      ...
      if (req._ingress?.metadata?.annotations?.['kubesail.com/agent-auth']) {
        ...
        if (query) {
          ...
          const { res } = await KubesailAgent.kubesailApiRequest('/agent/proxy-auth', 'POST', {
            agentKey: KUBESAIL_AGENT_KEY,
            agentSecret: KUBESAIL_AGENT_SECRET,
            nonce
          })
      ...
      const connect = async (group, version, kind) => {
        ...
        stream.on('data', async event => {
          ...
          if (event.object.kind === 'Secret') {
            ...
            } else if (event.object.type === 'kubernetes.io/tls') {
              const hostnames = (
            ...
            const appLabel = (
              (event.object?.metadata?.annotations || {})['kubesail.com/template'] || ''
            ).split(':')[0]
            if (appLabel) {
              if (event.type === 'ADDED') {
          ...
          if (filtered && userWantsEvent) {
            try {
              await KubesailAgent.kubesailApiRequest('/agent/event', 'POST', {
                agentKey: KUBESAIL_AGENT_KEY,
                agentSecret: KUBESAIL_AGENT_SECRET,
                event,
                retryLimit: 0,

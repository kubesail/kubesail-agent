...
class KubesailAgent {
  ...
  proxyRequestHandler = _protocol => async (req, res) => {
    logger.debug('proxyRequestHandler: start')
  ...
  async init() {
    ...
    this.httpsServer.on('upgrade', async (req, socket, head) => {
      ...
      http2Proxy.ws(req, socket, head, { ...head, ...settings })
    })
  ...
  async installCertificate(namespace, secretName, hostname, secret) {
    ...
    if (secret) {
      ...
      const certs = Buffer.from(secret.data['tls.crt'], 'base64')
        ...
        .replace(/\n\s/g, '')
        .split('\n')
      const key = Buffer.from(secret.data['tls.key'], 'base64').toString('ascii').replace(/\n\s/g, '')
      let ca
      if (certs.length > 1) ca = certs.pop()
      const context = { key, certs: certs.join('\n'), ca }
      logger.debug(
        `installCertificate: Installing TLS certificate from secret "${namespace}/${secretName}" for hostname "${hostname}"`,
        { replacing: !!this.reverseProxyTLSContexts[hostname] }
      )
  ...
  k8s = {
    ...
    findServiceForRequest: async (req, res) => {
      ...
      const ingresses = this.docs.filter(i => i.kind === 'Ingress' && i.spec.rules.find(r => r.host === host))
      const ingressWithPathMatch = ingresses.find(i =>
        i.spec.rules.find(r => r.http.paths.find(p => p.path && p.path === reqPath))
      )
          ...
          if (event.object.kind === 'Secret') {
            if (event.object.type === 'kubernetes.io/tls') {
              ...
              for (const hostname of hostnames) {
                if (event.type === 'ADDED') {
                  logger.debug('pre-install certificate:', {

  ...
  async init() {
    ...
    this.httpsServer.on('request', async (req, res) => {
      ...
      if (result?.ip && result?.port) {
        ...
        try {
          ...
          logger.error('Unknown error proxying request', {
            type: err.type,
            errMsg: err.message,
            stack: err.stack
          })
          return res.end('KS_INTERNAL_ERROR_002 UNKNOWN\n\n')
        }
    ...
    this.registerWithGateway().then(async () => {
      ...
      try {
        ...
        if (err.code === 404) {
          logger.silly('Failed to call installCertificate', { code: err.code })
  ...
  async registerWithGateway() {
    return new Promise((resolve, _reject) => {
      ...
      this.gatewaySocket.on('connect', async () => {
        ...
        logger.debug('Socket connected, emitting "init"')
      })

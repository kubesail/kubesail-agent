...
class KubesailAgent {
  ...
  proxyRequestHandler = _protocol => async (req, res) => {
    const result = await this.k8s.findServiceForRequest(req, res)
    if (result?.ip && result?.port) {
      // TODO: Force redirect if _protocol is http and app doesnt explicitly allow http
  ...
  createGatewaySocket = () => {
    ...
    logger.silly('createGatewaySocket: creating socket')
  ...
  async checkPublicIP() {
    try {
      const { _res, body } = await this.kubesailApiRequest('/whatsmyip', 'PUT', null, 0, 0, 5000, {
  ...
  async registerWithGateway() {
    return new Promise((resolve, _reject) => {
      ...
      logger.silly('Opening gateway socket')
      this.gatewaySocket.open()
    })
  }
    ...
    if (secret) {
      ...
      if (!secret.data['tls.crt'] || !secret.data['tls.key']) {
        ...
        this.reverseProxyTLSContexts[hostname] = tls.createSecureContext({
          key: Buffer.from(secret.data['tls.key'], 'base64'),
          cert: Buffer.from(secret.data['tls.crt'], 'base64')
        })
        logger.debug(
          `installCertificate: ${
  ...
  k8s = {
    ...
    init: async () => {
      ...
      // Pods
      const pods = await this.k8s.client.api.v1.endpoints.get()
    ...
    findServiceForRequest: async (req, res) => {
      ...
      logger.silly('Routing request:', { reqPath, ingresses: ingresses.map(i => i.metadata.name) })
      ...
      const sslRedirect = annotations['ingress.kubernetes.io/ssl-redirect']
      if (ingress?.spec?.tls && sslRedirect !== 'false' && req.protocol === 'http') {
        res.statusCode = 308
        res.setHeader('Location', 'https://' + host + reqPath)
        return res.end()
      }
      ...
      return { host, port, ip, protocol: backendProtocol, service, ingress, endpoint }
    },

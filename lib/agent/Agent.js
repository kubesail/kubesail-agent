  ...
  dns = {
    ...
    setResolver: async () => {
      ...
      try {
        ...
        this.dns.resolver.setServers(['1.1.1.1', '1.0.0.1'])
        logger.warn(
    ...
    lookup: (hostname, _opts, cb) => {
      if (isFQDN(hostname)) {
        this.dns.resolve4(hostname).then(results => {
          ...
          logger.silly('resolve4: resolved hostname', { hostname, results, chosen: host })
          cb(null, host, 4)
        }).catch(err => {
          logger.error('resolve4: Failed to lookup hostname', {
            hostname,
            errMsg: err.message,
            code: err.code
          })
        ...
        dns.lookup(hostname, { all: true }, (err, results) => {
          if (err) {
            logger.error('dns.lookup: Error looking up hostname', {
              hostname,
              errMsg: err.message,
              code: err.code
            })
          ...
          const host = sampleArray(results)
          if (!results[0] || !host) {
            logger.error('DNS lookup: no results for hostname', { hostname, results })
            return cb(new Error('Failed to lookup hostname'))
          }
          cb(null, results)
        })
      }
  ...
  kubesailApiRequest = (
    ...
    return new Promise((resolve, reject) => {
      ...
      const errHandler = e => {
        ...
        reject(
          new Error(
            `Failed to post to KubeSail API: ${method} ${KubeSailApiTarget}:${KubeSailApiPort || 443}${path}. Received "${e.code}" after ${retries} tries. Stack:\n${e.stack}`
          )
        )
      }
      logger.silly('kubesailApiRequest:', { path, method })
      const req = https.request({ ...options, path, agent: this.keepAliveAgentHTTPS }, res => {
  ...
  async installCertificate(namespace, secretName, hostname, secret) {
    ...
    if (!secret) {
      try {
        ...
        if (err.code !== 404) {
          logger.warn('installCertificate: Unable to get latest version of secret', { namespace, secretName, code: err.code })
        }
      }
    }
    if (secret) {

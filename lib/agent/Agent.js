...
class KubesailAgent {
  ...
  reverseProxyRequest = (req, options) => {
    ...
    return http2Wrapper.auto(`http://${options.hostname}:${options.port}`, {
      ...options,
      protocol: 'http:'
    })
  }
  reverseProxyResponse = (req, res, proxyRes) => {
    ...
    res.setHeader('x-powered-by', 'kubesail-agent')
  ...
  k8s = {
    ...
    findServiceForRequest: async (req, res) => {
      const host = (req.headers.host || '').split(':')[0] || (req.headers[':authority'] || '').split(':')[0]

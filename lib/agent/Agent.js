    ...
    this.httpsServer.on('upgrade', async (req, socket, head) => {
      ...
      http2Proxy.ws(req, socket, head, { ...head, ...settings }).catch(err => {
        ...
        if (['ECONNRESET', 'ECONNREFUSED'].includes(err.code)) {
          ...
          return res.end(`KS_WS_PROXY_APP_ERROR: ${err.code || 'UNKNOWN'}`)
        }
      })
    })
  ...
  async installCertificate(namespace, secretName, hostname, secret) {
    ...
    if (secret) {
      ...
      const certs = Buffer.from(secret.data['tls.crt'], 'base64')
        ...
        .split(beginCert)
        .filter(Boolean)
      const key = Buffer.from(secret.data['tls.key'], 'base64').toString('ascii').trim()

      let ca
      let cert
      if (certs.length > 1) {
        cert = beginCert + certs.shift()
        ca = certs.join(beginCert)
      } else {
        cert = beginCert + certs.join(beginCert)
      }

      const context = { key, cert, ca }

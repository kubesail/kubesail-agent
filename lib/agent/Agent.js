    ...
    setResolver: async () => {
      ...
      try {
        ...
        if (this.dns.usingFallback && resolved) {
          logger.info('DNS appears to be online! Reverting resolver back to defaults')
          this.status.gatewayTargets = resolved
          this.dns.usingFallback = false
          this.dns.resolver = new dns.Resolver()
          this.dns.resolve4 = promisify(this.dns.resolver.resolve4).bind(this.dns.resolver)
        } else {
        ...
        logger.warn(
          'Unable to resolve DNS - Falling back to CloudFlare DNS as backup! Please check your cluster for DNS capabilities! Trying again in 30 seconds',
          { errMsg: err.message, errCode: err.code }
        )
        setTimeout(() => this.dns.setResolver(), 30000)
      }
    },
  ...
  createGatewaySocket = () => {
    ...
    if (isIP(this.status.gatewayTargets[0])) {
      logger.warn(
        ...
        { resolvedGatewayTarget: this.status.gatewayTargets[0], port: KUBESAIL_AGENT_GATEWAY_PORT }
      )
      connectionOptions.insecure = true
      connectionOptions.rejectUnauthorized = false
    }

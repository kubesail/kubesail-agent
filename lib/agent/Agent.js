  ...
  k8s = {
    ...
    ensureHostPort: async () => {
      ...
      while (checkOnPod) {
        ...
        if (pod) {
          if (pod.body.status.phase === 'Failed' && pod.body.status.reason === 'NodePorts') {
    ...
    findServiceForRequest: async (req, res) => {
      ...
      if (
        ...
        if (res) {
          logger.debug('requestHandler: App not yet ready?', {
            endpoint,
      ...
      // Instead, we'll skip the service proxy and talk directly to the endpoint
      const ip = endpoint.subsets[0].addresses[0].ip
      const port = endpoint.subsets[0].ports[0].port
      return { host, port, ip, service, ingress }
    },
    watchResources: () => {

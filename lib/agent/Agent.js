...
class KubesailAgent {
  ...
  httpsServer = http2.createSecureServer({
    ...
    SNICallback: (domain, cb) => {
      if (this.reverseProxyTLSContexts[domain]) {
        ...
        if (this.reverseProxyTLSContexts[this.status.clusterAddress]) {
          return cb(null, this.reverseProxyTLSContexts[this.status.clusterAddress])
        }
        return cb(null, null)
      }
    }
  ...
  async checkK8g8CertificateExpiration (certificate) {
    ...
    const expiresIn = Date.now() - validUntil
    const needsRenewal = expiresIn > (1000 * 60 * 60 * 24)
    if (needsRenewal) {
      logger.warn(`checkK8g8CertificateExpiration: k8g8 certificate needs renewal! Requesting a new certificate.`)
      this.features.k8g8Cert = false
      this.gatewaySocket.emit('health-check', this.generateHealthCheckData())
    } else {
      logger.debug('checkK8g8CertificateExpiration: Certificate still valid for', { expiresIn })
    }
    return needsRenewal
  }

...
class KubesailAgent {
  ...
  httpsServer = https.createServer(
    ...
    async (req, res) => {
      ...
      if (result?.ip && result?.port) {
        ...
        this.reverseProxy.web(req, res, { target }, err => {
          const debugObj = {
            code: err.code,
            url: req.url,
            target,
  ...
  async init() {
    ...
    this.reverseProxy.on('proxyReq', function (proxyReq, _req, _res, _options) {
      proxyReq.setHeader('X-Proxy-Id', 'kubesail-agent')
    })
    this.reverseProxy.on('proxyRes', function (proxyRes, req, _res, _options) {
  ...
  k8s = {
    ...
    trackResource: (eventType, doc) => {
      ...
      if (['Ingress', 'Service', 'Endpoints'].includes(doc.kind)) {
        const docMatch = d =>
          d.kind === doc.kind &&
    ...
    findServiceForRequest: async (req, res) => {
      ...
      if (
        ...
        if (secret && secret?.data?.auth) {
          ...
          logger.warn(
            ...
            { secret: !!secret, secretName: annotations['ingress.kubernetes.io/auth-secret'] }
          )
          res.writeHead(401, { 'Content-Type': 'text/plain', 'WWW-Authenticate': 'Basic' })
          return res.end('Sorry, invalid credentials')
        }
      ...
      return { port: port.port, ip: service.spec.clusterIP, service, ingress }
    },
    watchResources: () => {
      const connect = async (group, version, kind) => {
        ...
        stream.on('data', async event => {
          ...
          if (
            this.usersWatchingEvents.find(u => u.namespace === event.object.metadata.namespace) &&
            this.k8s.filterKubeEvents(event)
          ) {
            logger.silly('Sending event to kubesail?', {
              watching: this.usersWatchingEvents,
              filter: this.k8s.filterKubeEvents(event)
            })
            try {

...
class KubesailAgent {
  ...
  async checkK8g8CertificateExpiration(certificate) {
    ...
    // Renew certs when they have 3 days left
    const renewsWhenCertExpiresIn = 1000 * 60 * 60 * (24 * 3)
    ...
    if (needsRenewal) {
      ...
      logger.debug('checkK8g8CertificateExpiration: Certificate still valid for', { expiresIn: expiresHuman, renewsWhenCertExpiresIn: renewHuman })
    }
    return needsRenewal
  }
      ...
      const connect = async (group, version, kind) => {
        ...
        stream.on('data', async event => {
          ...
          if (event.object.kind === 'Secret') {
            ...
            const appLabel = ((event.object?.metadata?.annotations || {})['kubesail.com/template'] || '').split(':')[0]
            if (appLabel) {
              if (event.type === 'ADDED') {

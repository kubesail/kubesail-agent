...
module.exports = function (proxy) {
  proxy.ws = function ws(req, socket, head, options, callback) {
    ...
    // Legacy compat...
    promise.then(() => callback(null, req, socket, head)).catch(err => callback(err, req, socket, head))
  }
  ...
  proxy.web = function web(req, res, options, callback) {
    ...
    // Legacy compat...
    promise.then(() => callback(null, req, res)).catch(err => callback(err, req, res))
  }

  async function compat(ctx, options) {
    const { req, res } = ctx

    const { hostname, port, path, socketPath, protocol, timeout, proxyTimeout, proxyName, onReq, onRes } =
      options

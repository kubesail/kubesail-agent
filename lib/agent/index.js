...
const { sampleArray, kubesailApiRequest } = require('../shared')
const {
...
class KsAgent {
  ...
  async writeAvahiHosts() {
    ...
    if (dbusAvailable && newLocalHostnames.length > 0) {
      const aRecordTarget = Uint8Array.from(process.env.NODE_IP.split('.'))
      logger.info(`Publishing .local DNS addresses (aliased to "${aRecordTarget}")`, {

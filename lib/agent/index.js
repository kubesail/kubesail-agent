...
class KsAgent {
  ...
  async installCertIssuer() {
    if (this.k8sClient.apis['cert-manager.io']) {
      ...
      try {
        ...
        if (clusterIssuer) {
          logger.info('Updating ClusterIssuer for LetsEncrypt!', { email: this.email })
  ...
  async findIngresses() {
    ...
    if (this.ingressApi) {
      ...
      await this.setIngressHostnames()
      // this.resources.ingresses.forEach(async ing => {
      //   if (
      //     this.features.ingressController === 'traefik' &&
      //     !(ing?.metadata?.annotations || {})['kubernetes.io/ingress.class']
      //   ) {
      //     if (!ing.metadata.annotations) ing.metadata.annotations = {}
      //     ing.metadata.annotations['kubernetes.io/ingress.class'] = 'public'
      //     await this.ingressApi
      //       .namespace(ing.metadata.namespace)
      //       .ingresses(ing.metadata.name)
      //       .put({ body: ing })
      //   }
      // })
    } else {

    ...
    return new Promise((resolve, reject) => {
      const requestHandler =
        ...
        (stream, { host }) => {
          ...
          } else if (this.firewall[host] && this.ingressEndpointMap[host]) {
            proxyTarget = this.ingressEndpointMap[host].ip
            port = this.ingressEndpointMap[host].port
            logger.silly('Forwarding request', {
              proxyTarget,
              host,
              port,
              clusterAddress: this.clusterAddress

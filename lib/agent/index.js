...
const { sampleArray, kubesailApiRequest, setPTimeout, loadSpec } = require('../shared')
const {
...
class KsAgent {
  ...
  http = http.createServer((req, res) => {
    logger.debug('HTTP server request?', req.path)
    if (this.agentReady) {
  ...
  async installTraefikMiddleware() {
    ...
    try {
      try {
        await helm.v1.namespaces('kube-system').helmchartconfig.post({
          body: {
      ...
      const agentNamespace = process.env.POD_NAMESPACE || 'kubesail-agent'
      try {
        await this.k8sClient.api.v1.namespaces(agentNamespace).services.post({
          body: {
            apiVersion: 'v1',
            kind: 'Service',
            metadata: { name: 'agent-redirect' },
            spec: {
              type: 'ClusterIP',
              selector: { app: 'kubesail-agent' },
              ports: [{ name: 'http', port: KUBESAIL_AGENT_HTTP_LISTEN_PORT, protocol: 'TCP' }]
            }
          }
        })
      } catch (err) {
      ...
      try {
        await this.k8sClient.api.v1.namespaces('default').services.post({
          body: {
            apiVersion: 'v1',
            kind: 'Service',
            metadata: { name: 'error-middleware-redirect' },
            spec: {
              type: 'ExternalName',
              externalName: `agent-redirect.${agentNamespace}.svc.cluster.local`,
              ports: [{ name: 'http', port: KUBESAIL_AGENT_HTTP_LISTEN_PORT, protocol: 'TCP' }]
            }
          }
        })
      ...
      try {
        await traefik.v1alpha1.namespaces('default').middlewares.post({
          body: {
            apiVersion: 'traefik.containo.us/v1alpha1',
            kind: 'Middleware',
            metadata: { name: 'error-page-redirect' },
            spec: {
              errors: {
                status: ['404'],
                query: '/{status}.html',
                service: {
                  name: 'error-middleware-redirect',
                  port: KUBESAIL_AGENT_HTTP_LISTEN_PORT
                }
              }
            }
          }
      ...
      logger.error('Failed to install Traefik middleware:', {
        errMsg: err.message,
        stack: err.stack,
        code: err.code
      })
    ...
    logger.debug('installTraefikMiddleware: Completed')
  }

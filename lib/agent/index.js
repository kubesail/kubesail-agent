...
logger.debug(`Agent bootup:`, { TLS_CERT_PATH })

// TODO: Remove this!
const connectionOptions = { insecure: true, rejectUnauthorized: false }

const connectionString = `${AGENT_GATEWAY_TARGET}?username=${KUBESAIL_AGENT_USERNAME ||
...
class KsAgent {
  ...
  agentHttpsReplyer = https.createServer(
    ...
    (req, res) => {
      if (req.url === '/503') res.sendStatus(503)
  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('agentData', agentData => {
        ...
        logger.info('Agent recieved configuration from Gateway!', {
          AGENT_GATEWAY_TARGET,
          agentData
        })
        resolve()
      ...
      this.gatewaySocket.on('config-request', () => {
        // Reply to gateway with kube config from k8sClient and k8sApiUrl! :D
        if (!this.k8sClient.backend) {
          this.gatewaySocket.emit('config-response', { error: true })
          return
        }
        const kubeAuth = {
          apiVersion: 'v1',
          clusters: [{ cluster: [Object], name: 'docker-desktop' }],
          contexts: [{ context: [Object], name: 'docker-desktop' }],
          'current-context': 'docker-desktop',
          kind: 'Config',
          preferences: {},
          users: [
            { name: 'docker-desktop', user: [Object] },
            { name: 'kubesail-erulabs', user: [Object] },
            { name: 'kubesail-pastudan', user: [Object] }
          ]
        }
        console.log('got config request, responding', this.k8sClient.backend)
        this.gatewaySocket.emit('config-response', { backend: this.k8sClient.backend })
      })

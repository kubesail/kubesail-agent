...
class KsAgent {
  ...
  async writeAvahiHosts() {
          ...
          if (rule.host.endsWith('.local')) {
            ...
            if (svcName) {
              ...
              if (svc && svc.spec.clusterIP) {
                localHostnames.push(
                  `\n# k8s Ingress: ${ing.metadata.name}.${ing.metadata.namespace}\n${svc.spec.clusterIP} ${rule.host}`
                )
              }
            }
          }
    ...
    if (fs.existsSync('/mnt/avahi/hosts')) {
      logger.debug('Writing Avahi hosts file')
      fs.writeFile('/mnt/avahi/hosts', localHostnames.join('\n'))

...
class KsAgent {
  ...
  async writeQRCode(data) {
    ...
    fs.stat(socketPath, (err, _stats) => {
      if (err) {
        return logger.silly('No framebuffer socket. Not drawing QR code')
      }
      try {
        const req = http.request(
          {
            socketPath,
            path: `/qr?content=${encodeURIComponent(data)}`,
            method: 'GET'
          },
          res => {
  ...
  async writeFramebufferImage(path) {
    ...
    fs.stat(socketPath, (err, _stats) => {
      if (err) {
        return logger.silly('No framebuffer socket. Not drawing image')
      }
      try {
        const req = http.request(
          {

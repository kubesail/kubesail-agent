  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      const requestHandler = protocol => (stream, { host }) => {
        ...
        socket.on('error', err => {
          logger.warn('requestHandler() error on socket:', { errMsg: err.message, type: err.type })
        })
        stream.on('error', err => {
          logger.warn('requestHandler() error on stream:', { errMsg: err.message, type: err.type })
        })
      }

    ...
    return new Promise((resolve, reject) => {
      const requestHandler = protocol => (stream, { host }) => {
        ...
        if (host === this.clusterAddress) {
          ...
          port = uri.port || 443
          logger.silly('Forwarding request to Kubernetes API', { proxyTarget, port })
          ...
          logger.silly('Forwarding request to Ingress Controller', { proxyTarget, port })
        }
      ...
      this.gatewaySocket.on('connect_error', error => {
        logger.warn('Disconnected from KubeSail Gateway!', {
          type: error.type,

...
class KsAgent {
  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      const requestHandler = protocol => (stream, { host }) => {
        ...
        stream.on('error', err => {
          logger.warn('requestHandler() error on stream:', { errMsg: err.message })
        })
      }
      ...
      this.gatewaySocket.on('config-request', () => {
        ...
        const kubeAuth = {
          ...
          kind: 'Config',
          preferences: {},
          users: [{ name: 'docker-desktop', user: [Object] }]
        }
        this.gatewaySocket.emit('config-response', { backend: kubeAuth })
      })
  ...
  async setIngressControllerEndpoint(endpoint /*: Object */) {
    if (endpoint && typeof endpoint === 'object' && Array.isArray(endpoint.subsets)) {
      ...
      logger.error('setIngressControllerEndpoint() recieved invalid endpoint object...', {
        endpoint
      })
    }
  }

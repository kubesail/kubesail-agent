...
const { sampleArray, writeHeader, kubesailApiRequest, encodeFQDN } = require('../shared')
const {
...
class KsAgent {
  ...
  async watchAll(docsToWatch /*: Array<Object> */) {
    ...
    const connect = async (group, version, kind) => {
      ...
      stream.on('data', async event => {
        ...
        if (!newVersion) {
          logger.error('Unexpected data from watch stream!', event)
          return
        }
  ...
  async writeAvahiHosts() {
    ...
    if (dbusAvailable && newLocalHostnames.length > 0) {
      logger.info(`Publishing .local DNS addresses (aliased to "${NODE_NAME}")`, {
        hosts: newLocalHostnames.map(l => l.rule.host)
      })
      newLocalHostnames.forEach(async l => {
        try {
          ...
          entryGroupInt.AddRecord(-1, -1, 0, l.rule.host, 0x01, 0x05, 60, encodeFQDN(NODE_NAME))
          entryGroupInt.Commit()
          this.localHostnameAvahiGroups[l.rule.host] = entryGroupInt
        } catch (err) {
          logger.error('Failed to write .local DNS addresses', err.message)
        }
      })
    }
  }

  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('error', error => {
        logger.warn('Socket error:', { type: error.type, code: error.description.code })
      })
      ...
      this.gatewaySocket.on('connect_error', error => {
        logger.warn('Socket connect_error:', { type: error.type, code: error.description.code })
      })
      this.gatewaySocket.on('connect_timeout', timeout => {
        logger.warn('Socket connect_timeout:', timeout)
      })

      logger.info('Connecting to gateway socket...', { gateway: KUBESAIL_AGENT_GATEWAY_TARGET })

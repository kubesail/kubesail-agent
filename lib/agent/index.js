  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('connect_error', error => {
        logger.error(
          `Disconnected from KubeSail Gateway ("${error.description.code}") Reconnecting...`,
          { KUBESAIL_AGENT_GATEWAY_TARGET, errorDetails: error.description }
        )
      })
      // this.gatewaySocket.on('connect_timeout', timeout => {
      //  logger.error('gatewaySocket connect_timeout:', timeout)
      //  throw new Error(`Gateway socket connect timeout after ${timeout}ms!`)
      // })

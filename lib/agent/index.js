  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('config-request', () => {
        ...
        this.gatewaySocket.emit('config-response', { kubeConfig: this.generateKubeConfig() })
      })
  ...
  async writeAvahiHosts() {
    ...
    if (fs.existsSync('/mnt/avahi/hosts')) {
      if (localHostnames.length > 0) {
        logger.debug(
          'Writing Avahi hosts file',
          localHostnames.map(l => l.rule.host)
        )
        const hostsFile = localHostnames.map(
          l =>
            `\n# k8s Ingress: ${l.ing.metadata.name}.${l.ing.metadata.namespace}\n${l.svc.spec.clusterIP} ${l.rule.host}`
        )
        fs.writeFile('/mnt/avahi/hosts', hostsFile.join('\n'), err => {
          if (err) logger.error('Failed to write Avahi hosts file: ', err.message)
        })
      }

...
class KsAgent {
  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('kube-watch', ({ username, startOrStop, resourcesToWatch }) => {
        if (startOrStop === 'start') {
          this.usersWatching.push(username)
        } else {
          this.usersWatching = this.usersWatching.filter(u => u !== username)
        }
      })

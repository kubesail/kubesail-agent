...
class KsAgent {
  ...
  async watchForIngresses() {
    ...
    if (this.ingressWatcherStream) this.ingressEndpointWatchStream.close()
    ...
    this.ingressWatcherStream.on('end', function () {
      logger.info('watchForIngresses: stream end, reconnecting', {})
      if (reconnectTimeout) return
      reconnectTimeout = setTimeout(() => {
        clearTimeout(reconnectTimeout)
        this.watchForIngresses()
    ...
    this.ingressWatcherStream.on('close', function () {

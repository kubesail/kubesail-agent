  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('connect', async () => {
        ...
        const { body } = await this.apiRequest(`/agent/version/${KUBESAIL_AGENT_KEY}`)
        const { versionRange, latest } = JSON.parse(body)
        const currentVersion = semver.clean(
          (
            await this.k8sClient.apis.apps.v1
              .namespaces('kubesail-agent')
              .deployments('kubesail-agent')
              .get()
          ).body.spec.template.spec.containers[0].image.split(':')[1]
        )

        if (semver.satisfies(currentVersion, versionRange)) {
          logger.info(
            `\u001b[32m✓\u001b[0m Your KubeSail Agent is up to date. Acceptable versions are ${versionRange} and you are running ${currentVersion}.`
          )
        } else {
          logger.info(
            `\u001b[31m✘\u001b[0m Your KubeSail Agent is out of date. Acceptable versions are ${versionRange} and you are running ${currentVersion}. Attempting to update to ${latest}...`
          )
            ...
            .patch({
              body: {
                spec: {
                  template: {
                    spec: {
                      containers: [{ name: 'agent', image: `kubesail/agent:${latest}` }]
                    }
                  }
                }
              }
      ...
      this.gatewaySocket.on('disconnect', reason => {
        if (reason === 'io client disconnect' || reason === 'io server disconnect')
          logger.error('Gateway closed connection')
        else logger.error('Disconnected from gateway!', { reason })

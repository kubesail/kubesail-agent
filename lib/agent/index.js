...
class KsAgent {
    ...
    const connect = async (group, version, kind) => {
      ...
      stream.on('data', async event => {
        if (event.type === 'ERROR' && event.object.code === 410) {
          logger.error('Got ERROR event type - reconnecting stream!', { event, resourceVersion })
          stream.destroy()
          await connect(group, version, kind)
  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('agent-data', agentData => {
        logger.debug('Agent recieved new agent-data from gateway!', {
      ...
      this.gatewaySocket.on('kube-watch', ({ username, namespace, startOrStop }) => {
        if (startOrStop === 'start') {
          logger.silly('User starting to watch namespace events', { username, namespace })
  ...
  endpointWatcherStreamVersion = undefined
  ...
  async watchForIngressEndpointChanges() {
    ...
    if (this.ingressEndpointWatchStream) {
      try {
        this.ingressEndpointWatchStream.destroy()
      } catch (err) {
        logger.warn('Failed to destroy existing ingressEndpointWatchStream', {
          errMsg: err.message
        })
      }
    }
    ...
    this.ingressEndpointWatchStream = await this.k8sClient.api.v1.watch
      ...
      .getObjectStream({
        qs: {
          timeoutSeconds: WATCH_STREAM_TIMEOUT_SECONDS,
          resourceVersion: parseInt(this.endpointWatcherStreamVersion, 10) || undefined
        }
      })
    ...
    this.ingressEndpointWatchStream.on('data', async event => {
      if (event.type === 'ADDED' || event.type === 'MODIFIED') {
        logger.debug(
          'watchForIngressEndpointChanges: Saw modification to ingress endpoint - swapping endpoint to:',
          { endpoint: event?.object?.metadata }
        )
        if (event.type !== 'ERROR') {
          const newVersion = parseInt(event.object.metadata.resourceVersion, 10)
          if (
            !this.endpointWatcherStreamVersion ||
            newVersion > this.endpointWatcherStreamVersion
          ) {
            this.endpointWatcherStreamVersion = newVersion
          }
        }
  ...
  ingressWatcherStream = null
  ingressWatcherStreamVersion = null
  ...
  async watchForIngresses() {
    ...
    if (this.ingressWatcherStream) {
      try {
        this.ingressWatcherStream.destroy()
      } catch (err) {
        logger.warn('Failed to destroy existing ingressWatcherStream', { errMsg: err.message })
      }
    }

    this.ingressWatcherStream = await this.ingressApi.watch.ingresses // .namespaces('*')
      .getObjectStream({
        qs: {
          timeoutSeconds: WATCH_STREAM_TIMEOUT_SECONDS,
          resourceVersion: this.ingressWatcherStreamVersion
        }
      })

    let reconnections = 0
    let reconnectTimeout
    this.ingressWatcherStream.on('error', err => {
    ...
    this.ingressWatcherStream.on('disconnect', () => {
    ...
    this.ingressWatcherStream.on('end', () => {
      logger.debug('watchForIngresses: stream end, reconnecting', {
        ingressWatcherStreamVersion: this.ingressWatcherStreamVersion
      })
      if (reconnectTimeout) return
      reconnectTimeout = setTimeout(() => {
        clearTimeout(reconnectTimeout)
    ...
    this.ingressWatcherStream.on('close', () => {
      logger.debug('watchForIngresses: stream closed')
    })

    this.ingressWatcherStream.on('data', async event => {
      if (event.type === 'ADDED') {
      ...
      const newVersion = parseInt(event.object.metadata.resourceVersion, 10)
      if (!this.ingressWatcherStreamVersion || newVersion > this.ingressWatcherStreamVersion) {
        this.ingressWatcherStreamVersion = newVersion
      }
      await this.setIngressHostnames()
    })
  }

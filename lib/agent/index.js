...
const METRICS_SCRAPE_INTERVAL_SECONDS = 5
    ...
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('pending', pendingKey => {
        if (pendingKey) {
          ...
          this.writeFramebufferImage('lib/agent/images/qr-setup-init.png')
          this.writeQRCode(`https://kubesail.com/qr/${pendingKey}`)
        } else {
      ...
      this.gatewaySocket.on('qr-scanned', pendingKey => {
        this.writeFramebufferAnimation('lib/agent/images/qr-setup-init.png')
      })
    ...
    return new Promise(async (resolve, _reject)=>{
      ...
      fs.stat(socketPath, (err, _stats) => {
        ...
        try {
          ...
          req.on('error', error => {
            logger.warn('Unable to write image to PiBox FrameBuffer:', { errMsg: error.message, path })
          })
          req.write(file)
          req.end()
        } catch (err) {
    ...
    return new Promise(async (resolve, _reject)=>{
      ...
      fs.stat(socketPath, (err, _stats) => {
        ...
        try {
          ...
          req.on('error', error => {
            logger.warn('Unable to write animation to PiBox FrameBuffer:', { errMsg: error.message, path })
          })
          req.write(file)
          req.end()
        } catch (err) {

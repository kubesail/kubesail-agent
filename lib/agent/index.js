...
class KsAgent {
  ...
  async statsOn(delay = 0) {
    ...
    const req = await this.fbRequest(`/stats/on`, 'POST')
    req.end()
  }

  async writeQRCode(data) {
    logger.silly('writeQRCode', { data })
    const req = await this.fbRequest(`/qr?content=${encodeURIComponent(data)}`)
    req.end()
  }

  async writeFramebufferImage(path) {
    const file = await readFile(path)
    const req = await this.fbRequest(`/image`, 'POST')
    req.on('error', error => {
  ...
  async writeFramebufferAnimation(path) {
    const req = await this.fbRequest('/gif', 'POST')
    req.on('error', error => {

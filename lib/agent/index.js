...
function trackResource(rawEvent, pile) {
  ...
  if (event.type === 'ADDED') {
    ...
    pile = pile.map(d =>
      d.metadata.name === name && d.metadata.namespace === ns ? event.object : d
    )
    ...
    async (req, res) => {
      ...
      if (
        ...
        if (!req.headers.authorization) {
          res.setHeader('WWW-Authenticate', 'Basic realm="Words", charset="UTF-8"')
          res.statusCode = 401
        ...
        if (secret && secret?.data?.auth) {
          const [username, password] = (
            Buffer.from(secret?.data?.auth, 'base64').toString('ascii') || ''
          ).split(':')
          ...
          console.log({ authUser, username, isMatch, password, authPass })

          if (authUser !== username || !isMatch) {
            res.setHeader('WWW-Authenticate', 'Basic realm="Invalid Credentials", charset="UTF-8"')
            res.statusCode = 401
      ...
      if (
        ...
        res.writeHead(404, { 'Content-Type': 'text/plain' })
        return res.end(`App is not online!`)
      }
  ...
  async watchAll(docsToWatch /*: Array<Object> */) {
    const connect = async (group, version, kind) => {
      ...
      stream.on('data', async event => {
        ...
        if (eventKind === 'Secret') {
          trackResource(event, this.resources.secrets)

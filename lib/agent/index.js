...
const { sampleArray, kubesailApiRequest, encodeFQDN } = require('../shared')
const {
...
class KsAgent {
  ...
  async init() {
    logger.info('kubesail-agent starting!', {
      ...
      version: RELEASE,
      gateway: [KUBESAIL_AGENT_GATEWAY_TARGET, KUBESAIL_AGENT_GATEWAY_PORT]
        .filter(Boolean)
        .join(':')
    })
    initProm()
    ...
    const connectionString = [
      ...
      `?username=${KUBESAIL_AGENT_USERNAME || ''}&key=${KUBESAIL_AGENT_KEY || ''}&secret=${
        KUBESAIL_AGENT_SECRET || ''
  ...
  async findCertManagerNamespace(namespaces) {
    ...
    if (namespaces.body.items.find(namespace => namespace.metadata.name === 'cert-manager')) {
      const { status } = await this.k8sClient.apis['apiextensions.k8s.io'].v1

...
class KsAgent {
  ...
  registerWithGateway() {
    return new Promise((resolve, reject) => {
      socketioStream(this.gatewaySocket).on('http', (stream, data) => {
        console.log('Agent got forwarded HTTP request?', data)

        stream.on('data', chunk => {
          console.log('stream does stuff?', chunk)
        })
        stream.on('end', chunk => {
          console.log('stream ends', chunk)
        })
      })
      socketioStream(this.gatewaySocket).on('https', (stream, data) => {
        console.log('Agent got forwarded HTTPS request?', data)
      })

      this.gatewaySocket.on('connect', () => {
        logger.debug('Agent successfully connected to gateway!', { AGENT_GATEWAY_TARGET })
        resolve()
      })

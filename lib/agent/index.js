...
class KsAgent {
  ...
  async init() {
    ...
    const connectionString = `https://${resolvedGatewayTarget}?username=${
      KUBESAIL_AGENT_USERNAME || ''
  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('connect', () => {
        logger.debug('Connected to gateway socket!', { gateway: KUBESAIL_AGENT_GATEWAY_TARGET })
      })
  ...
  async findMetricsServerEndpoints() {
    logger.silly('findMetricsServerEndpoints: start')
  ...
  async findIngresses() {
    logger.silly('findIngresses: start')
  ...
  hasWarnedAboutMissingAvahi = false
    ...
    this.resources.ingresses.forEach(
      ing =>
        ...
        ing.spec.rules.find(rule => {
          if (rule.host.endsWith('.local')) {
            const svcName = (rule?.http?.paths || [])[0]?.backend?.service?.name
            if (!svcName) return
            const svc = this.resources.services.find(
              svc =>
                svc.metadata.namespace === ing.metadata.namespace && svc.metadata.name === svcName
            )
            if (svc && svc.spec.clusterIP) {
              localHostnames.push({ ing, svc, rule })
            }
          }
        })
    )

    if (fs.existsSync('/mnt/avahi/hosts')) {
      if (localHostnames.length > 0) {
        logger.debug('Writing Avahi hosts file', { names: localHostnames.map(l => l.rule.host) })
        const hostsFile = localHostnames.map(l => {
          // This is fairly naive - in general should usually point to the node's LAN address.
          const targetAddress = process.env.NODE_IP || l.svc.spec.clusterIP || '0.0.0.0'
          return `\n# k8s Ingress: ${l.ing.metadata.name}.${l.ing.metadata.namespace}\n${targetAddress} ${l.rule.host}`
        })
        fs.writeFile('/mnt/avahi/hosts', hostsFile.join('\n'), err => {
          if (err) throw err
        })
      }
    } else if (!this.hasWarnedAboutMissingAvahi) {
      logger.warn(
        'Avahi was not detected: .local dns addresses will not work magically. Please install Avahi on the host system to enable .local mDNS!'
      )
      this.hasWarnedAboutMissingAvahi = true
    }
  }
  ...
  async setIngressHostnames() {
    ...
    const needsIngressUpdate = !isEqual(newIngressHostnames, this.ingressHostnames)
    logger.silly('setIngressHostnames:', { newIngressHostnames, needsIngressUpdate })

    ...
    const connect = async (group, version, kind) => {
      ...
      stream.on('data', async event => {
        ...
        if (eventKind === 'Endpoints') {
          ...
          if (eventName === 'ghost') {
            console.log('GHOST EVENT--', {
              event,
              subsets: JSON.stringify(event.object.subsets, null, 2),
              newEndpoints: JSON.stringify(
                this.resources.endpoints.find(r => r.metadata.name === 'ghost'),
                null,
                2
              )
            })

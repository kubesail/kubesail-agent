...
const redbird = require('../../modules/redbird')
...
const {
  ...
  KUBESAIL_AGENT_IGNORE_INGRESS_CONTROLLER,
...
function trackResource(rawEvent, pile) {
  ...
  if (event.type === 'ADDED') {
    ...
    if (!exists) return [...pile, event.object]
    return [...pile]
...
class KsAgent {
  ...
  ingressProxyHttp = http.createServer((req, res) => {
  ...
  async init() {
    logger.info(`kubesail-agent starting in "${LOG_LEVEL}" mode`, {
      ...
      ignoreClusterIngress: KUBESAIL_AGENT_IGNORE_INGRESS_CONTROLLER
    })
    initProm()
    ...
    this.ingressProxyHttp.listen(KUBESAIL_AGENT_INGRESS_CONTROLLER_HTTP_PORT, '127.0.0.1')
      ...
      const requestHandler =
        ...
        (stream, { host }) => {
          ...
          if (host === this.clusterAddress || ADDITIONAL_CLUSTER_HOSTNAMES.includes(host)) {
            ...
            if (!KUBESAIL_AGENT_IGNORE_INGRESS_CONTROLLER) {
              ...
              if (this.resources.ingressEndpoints.length) {
                ...
                logger.debug('Forwarding request to Cluster Ingress Controller', {
                  proxyTarget,
                  port
                })
              } else if (this.useNodeIPRouting && process.env.NODE_IP) {
                proxyTarget = process.env.NODE_IP
                logger.debug('Forwarding request to Cluster Ingress Controller via NodeIPRouting', {
                  proxyTarget,
                  port
                })
              } else {
            ...
            if (!proxyTarget) {
              proxyTarget = process.env.NODE_IP
              logger.debug('Using built-in ingress controller', {
                proxyTarget,
                port
              })
              if (protocol === 'https') {
                ...
                proxyTarget = '127.0.0.1'
                port = KUBESAIL_AGENT_INGRESS_CONTROLLER_HTTP_PORT
              }
            }
          }
          if (!proxyTarget) {

...
class KsAgent {
  ...
  async init() {
    ...
    setInterval(() => {
      this.gatewaySocket.emit('health-check', { agentVersion: RELEASE, automated: true })
    }, 60000)
  }
  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('health-check', () => {
        ...
        this.gatewaySocket.emit('health-check', { agentVersion: RELEASE })
      })
  ...
  async findIngressControllerEndpoints() {
    ...
    logger.debug('Setting INGRESS_CONTROLLER_ENDPOINT!', { body: endpoint.body })
    if (endpoint) {
  ...
  async setIngressControllerEndpoint(endpoint /*: Object */) {
    logger.debug('setIngressControllerEndpoint start:', { endpoint })
    if (endpoint && typeof endpoint === 'object' && Array.isArray(endpoint.subsets)) {
      ...
      logger.debug('setIngressControllerEndpoint end:', { endpoints: this.resources.endpoints })
    }
  }

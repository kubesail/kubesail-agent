...
class KsAgent {
      ...
      stream.on('data', async event => {
        if (event.type === 'ERROR' && event.object.code === 410) {
          ...
          if (event && event.object && event.object.metadata && event.object.metadata.namespace) {
            if (this.watchingNamespaces[event.object.metadata.namespace]) {
  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('disconnect', reason => {
        if (reason === 'io client disconnect') logger.silly('Gateway closed connection')
        else logger.info('Disconnected from gateway!', { reason })
      })
      this.gatewaySocket.on('connect_error', error => {
        logger.silly('Disconnected from KubeSail Gateway!', {
          type: error.type,
          code: error.description.code
        })
      })
      this.gatewaySocket.on('connect_timeout', timeout => {
        logger.warn('gatewaySocket connect_timeout:', timeout)
      })
    ...
    stream.on('data', async event => {
      ...
      if (event.type === 'MODIFIED') {
        this.ingresses = this.ingresses.map(ing => {
          if (
            ing.name === event.object.metadata.name &&
            ing.namespace === event.object.metadata.namespace
          ) {
            return event.object
          }
          return ing
  ...
  ingresses = []
  ingressHostnames = []
  async setIngressHostnames(force /*: boolean */ = false) {
    if (!this.clusterAddress) return
    const newIngressHostnames = Array.from(
      new Set(
        this.ingresses
    ...
    if (force || newIngressHostnames.length !== this.ingressHostnames.length) {
      this.ingressHostnames = newIngressHostnames
      logger.debug('Sending hostMappingRequest', { newIngressHostnames })
      const { statusCode } = await this.apiRequest('/agent/hostMappingRequest', 'POST', {
        agentKey: KUBESAIL_AGENT_KEY,
        agentSecret: KUBESAIL_AGENT_SECRET,

...
class KsAgent {
  ...
  async init() {
    ...
    const tryIngressControllerSearch = async () => {
      ...
      if (!this.ingressControllerNamespace || !this.ingressControllerEndpoint) {
        logger.warn(
          'Unable to determine Ingress Controller endpoint and namespace - trying again in 30 seconds. You can install an ingress controller via the KubeSail.com interface!'
        )
        setTimeout(async () => {
          await tryIngressControllerSearch()
        }, 30000)
      } else {
    ...
    const tryCertManagerIssuerCreation = async () => {
      if (this.k8sClient.apis['cert-manager.io']) {
        const clusterIssuers = await this.k8sClient.apis[
          'cert-manager.io'
        ].v1alpha3.clusterissuers.get()
        if (
          !clusterIssuers.body.items.find(item => item.metadata.name === 'kubesail-letsencrypt')
        ) {
          logger.info('Creating new ClusterIssuer for LetsEncrypt!')
          await this.k8sClient.apis['cert-manager.io'].v1alpha3.clusterissuers.post({
            body: {
              apiVersion: 'cert-manager.io/v1alpha3',
              kind: 'ClusterIssuer',
              metadata: { name: 'kubesail-letsencrypt', namespace: 'cert-manager' },
              spec: {
                acme: {
                  email: KUBESAIL_AGENT_EMAIL,
                  server: 'https://acme-v02.api.letsencrypt.org/directory',
                  privateKeySecretRef: { name: 'kubesail-letsencrypt' },
    ...
    const connect = async (group, version, kind, resourceVersion) => {
      ...
      stream.on('data', async event => {
        if (event.type === 'ERROR' && event.object.code === 410) {
          ...
          if (this.usersWatchingEvents.length > 0 && this.filterKubeEvents(event)) {
            ...
            if (
              ...
              logger.silly('watchAll: not sending event:', {
  ...
  filterKubeEvents = event => {
    if (event && event.object && event.object.metadata) {
      if (
        event.type === 'MODIFIED' &&
        event.object.kind === 'ConfigMap' &&
        event.object.metadata.name === 'ingress-controller-leader-nginx'
      )
        return false
      return true
    }

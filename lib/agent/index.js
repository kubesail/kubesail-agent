...
class KsAgent {
        ...
        if (event.type === 'ERROR' && event.object.code === 410) {
          ...
          if (event && event.object && event.object.metadata && event.object.metadata.namespace) {
            if (this.watchingNamespaces[event.object.metadata.namespace]) {
              this.sendEventToKubeSail(event)
            }
          }
        }
      })
  ...
  sendEventToKubeSail = async (event /*: Object */) => {
    const data = {
  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('connect_error', error => {
        logger.debug('Disconnected from KubeSail Gateway!', {
          type: error.type,
  ...
  ingressHostnames = []
  setIngressHostnames() {
    const newIngressHostnames = new Set(
      this.ingresses
    ...
    if (newIngressHostnames.length !== this.ingressHostnames.length) {

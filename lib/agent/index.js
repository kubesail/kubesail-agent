...
function trackResource(event, pile) {
  ...
  if (event.type === 'ADDED') {
    ...
    pile = pile.map(d =>
      d.metadata.name === name && d.metadata.namespace === ns ? event.object : ing
    )
  } else if (event.type === 'DELETED') {
    pile = pile.filter(d => d.metadata.name !== name && d.metadata.namespace !== ns)
  }
}
...
class KsAgent {
  ...
  ingressProxy = https.createServer(
    ...
    (req, res) => {
      ...
      logger.debug('ingressProxy request handler:', { host })
      ...
      if (!ingress || !rule) {
        ...
        return res.end('No app installed at this address')
      }
      ...
      if (!service) {
        res.writeHead(404, { 'Content-Type': 'text/plain' })
        return res.end(
          `Ingress exists but points at a service named "${serviceName}" that doesn't exist`
        )
      }
      ...
      //   return (
      //     selectorKeys.filter(key => service.spec.selector[key] === e.metadata.labels[key])
      //       .length === selectorKeys.length
      //   )
      // })

      // console.log(endpoint.subsets[0].addresses[0].ip, endpoint.subsets[0].ports[0].port)

      const routingTo = service.metadata.name + '.' + service.metadata.namespace + ':' + port.port
      console.log('found endpoint', routingTo)

      const pipe = http.request({
        path: req.path,
        method: req.method,
        headers: req.headers,
        host: service.metadata.name + '.' + service.metadata.namespace + ':' + port.port
      })
      pipe.on('response', function (pipedRes) {
        pipedRes.pause()
        res.writeHeader(pipedRes.headers)
        pipedRes.pipe(res)
        pipedRes.resume()
  ...
  async init() {
    ...
    await this.setDefaultNodeIpIngresses()
  ...
  async setDefaultNodeIpIngresses() {
    ...
    if (!this.ingressApi) return

    this.resources.nodes.forEach(node => {
      const internalIP = node.status.addresses.find(a => a.type === 'InternalIP').address
      const ingress = this.resources.ingresses.find(ing =>
        ing.spec.rules.find(r => r.host === internalIP)
      )
      if (ingress) return
      // this.ingressApi.namespace(process.env.POD_NAMESPACE || 'kubesail-agent').ingresses.post({
      //   body: {
      //     apiVersion: 'networking.k8s.io/v1',
      //     kind: 'Ingress',
      //     metadata: { name: `${node.metadata.name}-default` },
      //     spec: {
      //       rules: [
      //         {
      //           host: internalIP,
    ...
    const connect = async (group, version, kind) => {
      ...
      stream.on('data', async event => {
        ...
        if (eventKind === 'Ingress') {
          ...
          trackResource(event, this.resources.ingresses)
        ...
        if (eventKind === 'Service') {
          trackResource(event, this.resources.services)
        }

        // Keep track of selected Endpoints (metrics, ingress-controller, etc)
        if (eventKind === 'Endpoint') {
          trackResource(event, this.resources.endpoints)
        ...
        if (eventKind === 'Node') {
          trackResource(event, this.resources.nodes)
          return
        }

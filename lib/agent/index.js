...
class KsAgent {
  ...
  resources /*: { endpoints: Array<Object>, services: Array<Object>, certificates: Array<Object> } */ =
    { ingresses: [], endpoints: [], services: [], certificates: [] }
  ...
  // store the k8g8 wildcard cert
  k8g8Cert = null

  // A map of hostname -> endpoints to proxy to based on the Ingress, Service, and Endpoints documents on the cluster.
  ingressEndpointMap = {}
  ...
  createAgentProxy = () => {
    ...
    if (this.k8g8Cert) {
      key = Buffer.from(this.k8g8Cert.data['tls.key'], 'base64').toString()
      cert = Buffer.from(this.k8g8Cert.data['tls.crt'], 'base64').toString()
    }
    ...
    logger.debug(`createAgentProxy: Starting https proxy`)

    this.agentHttpsProxy = https.createServer({ key, cert, honorCipherOrder: true }, (req, res) => {
      console.log('hrmmm no connect')
      res.end('foo')
    })

    this.agentHttpsProxy.on('connect', (req, clientSocket, head) => {
    ...
    return this.agentHttpsProxy
  }

  agentHttpsProxy = null
  ...
  async init() {
    ...
    await this.createAgentProxy()

    this.agentHttpsProxy.listen(INTERNAL_HTTPS_PROXY_PORT_503, '127.0.0.1')
  ...
  async watchAll(docsToWatch /*: Array<Object> */) {
    const connect = async (group, version, kind) => {
      ...
      function trackResource(resource, event) {
        const name = event.object.metadata.name
        const ns = event.object.metadata.namespace
        if (event.type === 'ADDED') {
          const exists = resource.find(r => r.metadata.name === name && r.metadata.namespace === ns)
          if (!exists) return [...resource, event.object]
        } else if (event.type === 'MODIFIED') {
          return resource.map(r => {
            if (r.metadata.name === name && r.metadata.namespace === ns) {
              return event.object
            }
            return r
          })
        } else if (event.type === 'DELETED') {
          return resource.filter(r => r.metadata.name !== name && r.metadata.namespace !== ns)
        }
        return resource
      }
      ...
      stream.on('data', async event => {
        ...
        if (
          ...
          if (event.type === 'ADDED') {
            ...
            this.k8g8Cert = event.object
            this.createAgentProxy()
          } else if (event.type === 'DELETED') {
        ...
        if (eventKind === 'Ingress') {
          this.resources.ingresses = trackResource(this.resources.ingresses, event)
          this.buildIngressEndpointMap()
          await this.setIngressHostnames()
        } else if (eventKind === 'Service') {
          this.resources.services = trackResource(this.resources.services, event)
          this.buildIngressEndpointMap()
        } else if (eventKind === 'Endpoint') {
          this.resources.endpoints = trackResource(this.resources.endpoints, event)
          if (
            eventNamespace === 'kube-system' &&
          ...
          this.buildIngressEndpointMap()
          return // Don't continue on to send events
        }
  ...
  buildIngressEndpointMap = () => {
    const newIngressMap = {}
    this.resources.ingresses.forEach(ing => {
      const annotations = ing?.metadata?.annotations || {}
      if (
        annotations['kubernetes.io/ingress.class'] &&
        annotations['kubernetes.io/ingress.class'] !== 'kubesail'
      ) {
        logger.silly(
          `buildIngressEndpointMap: Ignoring ingress ${ing?.metadata?.namespace}/${ing?.metadata?.name} due to ingress.class annotation`
        )
        return
      }
      const rules = ing?.spec?.rules || []
      rules.forEach(rule => {
        if (this.firewall[rule.host] && rule?.http?.paths?.[0]) {
          const svcTarget = rule.http.paths[0].backend.service
          if (!svcTarget) {
            logger.debug(`buildIngressEndpointMap: invalid service target: ${rule.host}`, {
              backend: rule.http.paths[0].backend
            })
            return
          }
          const svc = this.resources.services.find(
            s =>
              s.metadata.name === svcTarget.name && s.metadata.namespace === ing.metadata.namespace
          )
          if (svc) {
            const port = svc.spec.ports.find(
              p => p.name === svcTarget.port.name || p.port === svcTarget.port.number
            )
            if (port) {
              newIngressMap[rule.host] = {
        ...
        } else if (rule.host.endsWith('.local')) {
          // TODO: Support local only connections to .local addr
        } else {
          logger.debug(`buildIngressEndpointMap: ignoring host ${rule.host}: not in firewall`)
        }
      })
    })
    this.ingressEndpointMap = newIngressMap
  }
      ...
      const requestHandler =
        ...
        (stream, { host }) => {
          ...
          if (host === this.clusterAddress || ADDITIONAL_CLUSTER_HOSTNAMES.includes(host)) {
            ...
            port = uri.port || '443'
            // Or forward to the Ingress system
          } else {
            proxyTarget = '127.0.0.1'
            port = INTERNAL_HTTPS_PROXY_PORT_503
          }
          ...
          if (!port || !proxyTarget) {
            logger.error("Couldn't find proxyTarget for request", {
              host,
              port,
              proxyTarget,
  ...
  async findServices() {
    const services = await this.k8sClient.api.v1.services.get()
    this.resources.services = services.body.items
  }

  // Tries to find a metrics-server endpoint for monitoring & charting purposes
  async findMetricsServerEndpoints() {
    this.resources.endpoints.find(endpoint => {

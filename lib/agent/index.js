  ...
  async registerWithGateway() {
    return new Promise((resolve, reject) => {
      ...
      this.gatewaySocket.on('disconnect', reason => {
        throw new Error(`Gateway Socket disconnected! Reason: "${reason}"`)
      })
      this.gatewaySocket.on('connect_error', error => {
        throw new Error(`Unable to connect to Gateway! Error: "${error.description.code}"`)
      })
      this.gatewaySocket.on('connect_timeout', () => {
        throw new Error('Gateway Socket timed out!')
      })

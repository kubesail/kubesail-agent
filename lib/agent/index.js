...
const METRICS_SCRAPE_INTERVAL_SECONDS = 6
...
class KsAgent {
  ...
  async init() {
    ...
    let spec
    try {
      spec = await this.k8sClient.backend.http({ method: 'GET', pathname: '/openapi/v2' })
      ...
      throw new Error(
        'Unable to communicate with Kubernetes - possibly the cluster is not healthy? Cannot continue :('
      )
    }
    if (spec) {
      try {
    ...
    fs.watchFile('/var/run/secrets/kubernetes.io/serviceaccount/token', () => {
      logger.debug('ServiceAccount Token has changed! Emitting refreshed auth to KubeSail')
  ...
  async watchAll(docsToWatch /*: Array<Object> */) {
    const connect = async (group, version, kind) => {
      ...
      stream.on('data', async event => {
        ...
        if (eventKind === 'Ingress') {
          if (event.type === 'ADDED') {
            ...
            if (!exists) {
              ...
              this.resources.ingresses.push(event.object)
            }
        ...
        if (eventKind === 'Node') {
          if (event.type === 'ADDED') {
            const exists = this.resources.nodes.find(
              n => n.metadata.name === eventName && n.metadata.namespace === eventNamespace
            )
            if (!exists) this.resources.nodes.push(event.object)
          } else if (event.type === 'MODIFIED') {
            this.resources.nodes = this.resources.nodes.map(n => {
              if (n.metadata.name === eventName && n.metadata.namespace === eventNamespace) {
                return event.object
              }
              return n
            })
          } else if (event.type === 'DELETED') {
            this.resources.nodes = this.resources.nodes.filter(
              n => n.metadata.name !== eventName && n.metadata.namespace !== eventNamespace
            )
          }
          return
        ...
        (stream, { host }) => {
          ...
          if (host === this.clusterAddress || ADDITIONAL_CLUSTER_HOSTNAMES.includes(host)) {
            ...
            if (this.resources.endpoints.length) {
              ...
              logger.warn('Received request, the Ingress controller is offline or not installed!', {
                host,
      ...
      this.gatewaySocket.on('agent-data', async agentData => {
        ...
        if (!encounteredError) {
          if (agentData.refreshCredentials) {
            logger.debug('Gateway asked us to refresh credentials')
            this.gatewaySocket.emit('config-response', {
              kubeConfig: this.generateKubeConfig(),
              assertUsers: false
            })
  ...
  async writeQRCode(data) {
    ...
    fs.stat(socketPath, (err, _stats) => {
      if (err) return logger.silly('No framebuffer socket. Not drawing QR code')
      try {
        const reqPath = `/qr?content=${encodeURIComponent(data)}`
        const req = http.request({ socketPath, path: reqPath, method: 'GET' }, res => {
          logger.debug('PiBox Framebuffer response:', { status: res.statusCode })
        })
        req.on('error', error => {
          logger.warn('Unable to write to PiBox FrameBuffer:', { errMsg: error.message })
        })
        req.end()
      } catch (err) {
        logger.error('Failed to write QR code!', {
          errMsg: err.message,
          name: err.name,
          type: err.type
        })
  ...
  async statsOn(delay = 0) {
    logger.debug('PiBox stats on', { delay })
    setTimeout(() => {
      try {
        const socketPath = '/var/run/pibox/framebuffer.sock'
        const req = http.request({ socketPath, path: `/stats/on`, method: 'POST' })
        req.end()
      } catch (err) {
        logger.debug('PiBox stats on failed:', { errMsg: err.message })
      }
    }, delay * 1000)
  }
  ...
  async writeFramebufferImage(path) {
    return new Promise(async (resolve, _reject) => {
      logger.silly('writeFramebufferImage', { path })
      const socketPath = '/var/run/pibox/framebuffer.sock'
      const file = await readFile(path)

      fs.stat(socketPath, (err, _stats) => {
        if (err) return logger.silly('No framebuffer socket. Not drawing image')
        try {
          const req = http.request({ socketPath, path: `/image`, method: 'POST' }, res => {
            logger.silly('PiBox Framebuffer response:', { status: res.statusCode })
            resolve()
          })
          req.on('error', error => {
            logger.warn('Unable to write image to PiBox FrameBuffer:', {
  ...
  async writeFramebufferAnimation(path) {
    return new Promise(async (resolve, _reject) => {
      logger.silly('writeFramebufferAnimation', { path })
      const socketPath = '/var/run/pibox/framebuffer.sock'
      const file = await readFile(path)
      ...
      fs.stat(socketPath, (err, _stats) => {
        if (err) return logger.silly('No framebuffer socket. Not drawing image')
        try {
          const req = http.request({ socketPath, path: `/gif`, method: 'POST' }, res => {
            logger.debug('PiBox Framebuffer response:', { status: res.statusCode })
            resolve()
          })
          req.on('error', error => {
            logger.warn('Unable to write animation to PiBox FrameBuffer:', {
              errMsg: error.message,
              path
            })
          })
          ...
          logger.error('Failed to write animation!', {
            errMsg: err.message,
            name: err.name,
            type: err.type
          })
  ...
  async findMetricsServerEndpoints() {
    ...
    if (!foundMetricsServer) {
      logger.debug(
        'findMetricsServerEndpoints: Unable to find metrics server endpoint. Metric collection disabled'
      )
    }
  }
  ...
  async findIngresses() {
    ...
    if (this.ingressApi) {
      ...
      logger.debug('findIngresses: calling setIngressHostnames')
      await this.setIngressHostnames()
    } else {
      let debugKeys = {}
      try {

...
class KsAgent {
  ...
  async init() {
    ...
    const tryIngressControllerSearch = async () => {
      ...
      if (!this.ingressControllerNamespace || !this.ingressControllerEndpoint) {
        logger.warn(
          `Unable to determine Ingress Controller endpoint and namespace - trying again in ${Math.floor(
            SEARCH_INTERVAL_TIME_MS / 1000
          )} seconds. You can install an ingress controller via the KubeSail.com interface!`
        )
        setTimeout(tryIngressControllerSearch, SEARCH_INTERVAL_TIME_MS)
      } else {
    ...
    const retryCertManagerIssuerCreation = () => {
      setTimeout(async () => {
        ...
        const spec = await this.k8sClient.backend.http({ method: 'GET', pathname: '/openapi/v2' })
        this.k8sClient.templatedEndpoints = []
        this.k8sClient._addSpec(spec.body)
  ...
  async doIngressSearch(namsepace, endpoint) {
  ...
  async findIngressControllerEndpoints() {
    ...
    for (let i = 0; i < namespaces.length; i++) {
      const endpoint = await this.doIngressSearch(namespaces[i], endpoints[i])
      if (endpoint) {

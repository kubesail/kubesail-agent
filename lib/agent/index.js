...
class KsAgent {
  ...
  constructor() {
    ...
    const cleanup = () => {
      const keys = Object.keys(this.localHostnameAvahiGroups)
      for (k in keys) {
        this.localHostnameAvahiGroups[keys[k]].Reset()
        this.localHostnameAvahiGroups[keys[k]].Free()
      }
  ...
  async writeAvahiHosts() {
    ...
    if (newLocalHostnames.length > 0) {
      newLocalHostnames.forEach(l => {
        logger.info('Publishing local address:', { host: l.rule.host })
        ...
        entryGroupInt.AddAddress(-1, -1, 0, l.rule.host, process.env.NODE_IP)
